...
const model=process.env.MOTE_PRIVACY_MODEL;
const base=process.env.MOTE_PRIVACY_BASE_URL??'http://127.0.0.1:11434/v1';
...
const policy=process.env.MOTE_PRIVACY_POLICY||'Locate personal secrets, authentication codes, passwords, account/card numbers and private contact information. Mask them with tight but complete rectangles. If uncertain whether it can be safely redacted, deny the entire image.';
...
const port=Number(process.env.MOTE_PRIVACY_PORT??47833);
