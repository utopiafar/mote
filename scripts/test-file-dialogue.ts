...
const audio=readFileSync(resolve(process.argv[2]??'.mote/processing-validation/generated-interview.wav'));
...
assert.equal(detail.job.state,'succeeded',JSON.stringify(detail.job));assert.equal(detail.job.local_only,1);assert.notEqual(detail.job.summary_state,'succeeded');
