...
const config=structuredClone(pkg.build),temp=join(tmpdir(),'mote-mac-release-'+randomUUID());mkdirSync(temp,{mode:0o700});
try{
  ...
  const app=resolve(`apps/desktop/release/${process.arch==='arm64'?'mac-arm64':'mac'}/Mote Collector.app`);
  ...
  if(field('CFBundleIdentifier')!=='dev.mote.collector'||field('CFBundleShortVersionString')!==pkg.version)throw Error('Packaged Mac identity mismatch');
