...
const fixtureModel=createServer(async(req,res)=>{
  ...
  const body=JSON.parse(raw);assert.deepEqual(body.tools.map((t:any)=>t.function.name).sort(),['activity','changes','devices','evidence','file_chunks','media_activity','memories','progress_update','read_file_evidence','read_image','search_context','skill','source_history','source_items','sources','timeline']);
