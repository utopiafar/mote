...
async function run() {
  ...
  assert.equal(failedConversation.turns.at(-1).question, 'fixture-failure', 'failed question is retained');
  ...
  await js(`Array.from(document.querySelectorAll('.conversation-item')).find(b=>b.textContent.includes('合成计划')).click()`); await readyTurns(3);
  await input('继续刚才的计划'); await click('发送问题'); await readyTurns(4);
