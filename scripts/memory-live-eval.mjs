...
let relay,node;
const replay=process.env.MOTE_MEMORY_REPLAY_ALL?JSON.parse(await readFile(process.env.MOTE_MEMORY_REPLAY_ALL,'utf8')):null;
if(replay){report.mode='offline replay of previously generated live outputs';report.transport='Saved GPT-5.6-luna outputs → current host validation and lifecycle; no inference';}
try{
  const fixture=await seedMemoryFixtures(store);report.fixture=fixture.counts;console.log('Seeded',fixture.counts);
  relay=replay?{model:report.model,baseUrl:'http://127.0.0.1:1/v1',apiKey:'offline-fixture',close:async()=>{}}:await startCodexRelay({maxCalls:20,onCall:call=>{report.calls.push(call);console.log('MODEL',JSON.stringify({...call,toolCalls:undefined,content:call.content?'[saved in synthetic report]':undefined}));}});
  const fromCaptured=async(path,index)=>{const capture=JSON.parse(await readFile(path,'utf8'));assert.equal(capture.model,report.model);assert.equal(capture.effort,'max');return capture.calls.find(c=>c.content&&(index===undefined||c.index===index)).content;};
  const decode=content=>{const ids=JSON.parse(content).citationIds;return {...parseAnswer(content,new Map(ids.map(id=>[id,store.evidence([id])[0]]))),trace:[],runId:randomUUID()};};
  node=await buildApp({dataDir:directory,token:'generated-live-fixture',tokenPath:'fixture',host:'127.0.0.1',port:0,maxStorageBytes:100000000,maxExportBytes:10000000,retentionDays:0,insightIntervalHours:0,allowedOrigins:[],modelProvider:'openai',modelProtocol:'openai-completions',model:relay.model,modelBaseUrl:relay.baseUrl,apiKey:relay.apiKey,allowUnauthenticatedLocal:false,modelReasoningEffort:'max',modelMaxTokens:32768,modelTimeoutMs:600000,embeddingModel:'',embeddingBaseUrl:'',embeddingApiKey:'',diagnosticsEnabled:false},{store,createModelAgent:async(settings,reader)=>{const runtime=replay?{configured:true,close:async()=>{},query:async input=>{if(input.skill==='memory-extraction')return decode(await fromCaptured(process.env.MOTE_MEMORY_REPLAY_EXTRACTION));if(input.skill==='memory-consolidation')return decode(await fromCaptured(process.env.MOTE_MEMORY_REPLAY_CONSOLIDATION,4));if(input.skill==='working-memory')return {answer:replay.checks.find(c=>c.name==='working-summary').summary.text,citations:[],trace:[],runId:randomUUID()};if(input.skill==='personal-insight')return decode(replay.calls.findLast(c=>c.content).content);return replay.queryResult;}}:await createModelAgent(settings,reader);return {configured:runtime.configured,close:()=>runtime.close(),query:async input=>{try{return await runtime.query(input);}catch(error){console.error('HARNESS',error.name,error.message,error.reason);throw error;}}};}});
  ...
  let job;
  if(process.env.MOTE_MEMORY_REPLAY_EXTRACTION&&!replay){const captured=JSON.parse(await readFile(process.env.MOTE_MEMORY_REPLAY_EXTRACTION,'utf8')),sample=captured.calls.find(c=>c.content);assert.equal(captured.model,relay.model);assert.equal(captured.effort,'max');const parsed=parseAnswer(sample.content,new Map(anchorIds.map(id=>[id,store.evidence([id])[0]])));const saved=node.memories.extract({...parsed,trace:[],runId:randomUUID()},relay.model,{skillVersion:'memory-extraction@1.1.0'});job={status:'completed',memoryIds:saved.items.map(m=>m.id)};report.extractionGeneration='Previously captured live output, replayed after fixing stream timeout; not a new model generation';}
  ...
  if(process.env.MOTE_MEMORY_REPLAY_CONSOLIDATION&&!replay){
