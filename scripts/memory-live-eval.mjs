...
try{
  ...
  relay=await startCodexRelay({maxCalls:20,onCall:call=>{report.calls.push(call);console.log('MODEL',JSON.stringify({...call,content:call.content?'[saved in synthetic report]':undefined}));}});
  ...
  const p=node.lifecycle.settings();p.consolidation={enabled:true,intervalHours:1/60,minChanges:1,maxItems:30};node.lifecycle.configure(p);
  ...
  const result=await node.agent.query({question:'请用 memories 全文检索与原文核对：青岚项目从3月到8月的数据库选择是否变化、当前是什么？灯塔4月12日的验证是否完成？林岚的清晨开会偏好能算成陈禾的吗？请保留每个结论的原始引用。只回答这三个问题。',timeZone:'Asia/Shanghai'});
