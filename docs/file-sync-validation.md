# 0.0.18 Cordis 与录音处理增量验收

2026-09-16：拉取并合并最新主线后重新验证，以下全部使用生成内容。发布前额外修复 Android 10–12 的回执读取 API 兼容性；release/development 两个变体 lint 与 126 项单元测试通过，模拟器同步复测 **10.091 秒**通过，3 原件与 205 个 Shadow 再次核验正确。
