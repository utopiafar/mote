...
7. 最终 JPEG（默认质量 75，可配 40–95）通过 stdin 交给 Apple Vision 做中英文 OCR。OCR 只读最终遮挡图；开启“仅充电时 OCR”且未接电时，先保存为待处理，接电后再读取此 JPEG 补识别。关闭 OCR 时仍会归档图像，但中央 Agent 主要读取 OCR 与应用元数据，尚未识别的图中文字不会自动进入检索。
...
可以显式选择电池供电暂停、低电量暂停（0 关闭）、JPEG 质量和截图分辨率。已有采样间隔、空闲阈值、CPU 线程及审查图片尺寸也影响资源占用。电量策略默认不启用；启用后若电量无法确认则保守暂停。保存设置时应用自动结束旧规则下的处理并应用新参数，保留原采集开停状态，避免同一帧混用不同策略。
