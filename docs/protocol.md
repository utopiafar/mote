...
`MOTE_MODEL_REASONING_EFFORT` is `auto|off|low|high|max` for HTTP providers (DeepSeek defaults to `high`, others to `auto`); Codex App Server also accepts `minimal|medium|xhigh|ultra` when the selected model lists them. `MOTE_MODEL_MAX_TOKENS` defaults to 65536 and is bounded to 1–128000. Codex manages its own output budget. See [model configuration](model-providers.md). Reasoning is performed by the configured model through the Harness; no reasoning transcript is exposed as a user-facing answer. [DeepSeek's official thinking-mode documentation](https://api-docs.deepseek.com/guides/thinking_mode/) describes the provider controls and multi-round tool support. Higher effort can increase latency and usage.
...
`GET /api/capture-browser/sessions` accepts required ISO `after`/`before` (inclusive/exclusive, at most 32 days), optional `deviceId`, opaque `cursor`, and `limit` (1–60, default 20). It groups same-device, same-app consecutive screenshot samples; app switches, unknown identity or gaps greater than 300,000 ms split groups. Responses contain `items`, `totalCount`, `sessionCount`, `gapMs` and `nextCursor`. Session rows use the album row fields below, but `id` is the first capture ID and the range is the observed group range. Add `sessionId` to retrieve that group's exact image references with `totalCount` and `nextCursor`. Keep the original date/device scope unchanged when selecting a group or paging. The server groups before pagination and resolves membership by timestamp/ID order, including app switches at equal timestamps. Collector access remains restricted to its own device.
