...
内存写入队列最多保留 1024 条待写事件，另有最多 64 条正在写入；单条结构化日志最多 20 KiB，超出时截断可选字段并标记 `truncated`，不会丢弃整条事件。突发流量或磁盘故障时允许丢弃诊断事件，`droppedEvents` 和 `writeFailures` 会增加，业务隐私检查与入库不因此被绕过。关闭服务会等待当前日志写入结束。禁用诊断不会主动删除已经存在的历史日志文件。
