...
- **请求 / 收到 / 保存 / 拦截**分别计数。已保存只表示加密入队成功，已确认上传只在节点 ACK 匹配并清除本机记录后增加；截图和随手记分别统计。重复入队或重复 ACK 不重复计数。用户规则导致的暂停、收到画面后丢弃和实际处理失败不是同一件事。
...
0.6.0 / versionCode 8 的日常 debug、Dev 和测试 APK 构建通过。44 项 JVM 测试全部通过，无失败或跳过；lint 为 debug 0 error / 99 warning、Dev 0 error / 103 warning。两包通过 16 KiB ZIP 对齐及实际 APK 签名校验，继续使用上文兼容证书。下表为本机调试产物，不能代替正式 release 的签名发布清单：
