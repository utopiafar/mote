...
已登记设备不能通过普通新设备邀请接管。页面可以复制本机设备 ID，在中央选择或明确绑定这个 ID 生成邀请，不需要轮换设备 ID。凭据撤销、过期配置或离线期间认证失败时，先停止采集，再用**同一节点、绑定同一设备的邀请**确认“使用新凭据继续同步本机待上传截图/笔记/来源”；队列、已准备的随手记、设备身份和采集设置原样保留。已有记录绑定节点后，只要仍有队列、已准备的笔记或来源待发版本，就拒绝改投另一节点；先清空地址也不会解除这项绑定。首次未绑定的离线记录可在上述明确确认后绑定。正在采集、扫描或上传时也拒绝连接变更；已经发出的投屏授权回调在配置变化后不会启动旧会话。
