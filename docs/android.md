...
1. 在中央前端打开设备连接，生成 10 分钟有效、只能兑换一次的邀请。手机首页打开“连接中央节点”，可以扫描二维码、粘贴邀请 JSON / `mote://connect` URI，或通过系统选择器读取 JSON 文件。二维码解码器随 APK 打包，不依赖 Google Play；只有点击扫码入口后才请求相机权限，不保存相机画面。
2. 页面先显示解析出的节点地址与到期时间，再由用户确认连接。外部 URI 只打开审阅页面，不会自动兑换、保存或启动采集。邀请只接受 HTTPS 节点；本机调试例外仅允许 debug / Dev 用户显式启用 loopback HTTP。`localhost` 指手机自身，远程节点需要手机可达且受信任的 HTTPS 地址。
3. 邀请为这台设备签发仅用于采集和自身来源同步的凭据；不能读取全仓库、管理节点或向 Agent 查询。客户端校验兑换响应和 `/api/connections/self` 的节点、凭据、设备 ID、平台与写入能力后保存。可点击“测试已保存连接”检查身份；状态代表最后一次检查，并非永久在线保证。
4. 配置采集间隔（默认 30 秒）、队列上限（默认 256 MiB）、非计费 Wi-Fi 上传、排除包名、固定遮罩。未配置遮罩时不会声称自动脱敏。默认启用本机 Qwen 图片过滤；先下载或导入双模型，未就绪时不会截图。
5. 允许通知，在系统中启用“Mote 屏幕采集”无障碍服务，返回应用点击开始。服务授权与开始采集是两个独立步骤。连接和配置页带 `FLAG_SECURE`，避免凭据被采集。通知和应用内均可停止；停止截图后已有队列继续同步。
6. 打开“采集与存储详情”查看已保存、被拦截、失败、待上传和已确认结果。已同步资料、Agent 查询与便携归档在中央节点管理，手机加密队列是上传缓冲区。
...
已登记设备不能通过普通新设备邀请接管。页面可以复制本机设备 ID，在中央选择或明确绑定这个 ID 生成邀请，不需要轮换设备 ID。凭据撤销、过期配置或离线期间认证失败时，先停止采集，再用**同一节点、绑定同一设备的邀请**确认“使用新凭据继续同步本机待上传截图/笔记/来源”；队列、已准备的随手记、设备身份和采集设置原样保留。更换节点地址时只要还有队列、已准备的笔记或来源待发版本，就会拒绝操作。正在采集、扫描或上传时也拒绝连接变更；已经发出的投屏授权回调在配置变化后不会启动旧会话。
...
首页可以设置默认采集级别，再逐行输入 `包名=content`、`包名=activity` 或 `包名=off` 覆盖默认值。最多 200 项；按完整包名精确匹配，不按名称或正文关键词推断。旧的排除包名列表始终按 `off` 执行，优先于分级设置。内容采集时，可见键盘和系统辅助窗口也遵守同一规则，没有内置系统白名单；默认 `off` 时，只有用户明确将已识别辅助窗口设为 `content` 才允许内容样本。活动采样本来不读取任何窗口内容，无需把已识别辅助窗口设为 `content`，但未知窗口、多应用及旧排除仍暂停。
...
如果全部规则都不需要内容，即使保存的是投屏模式，也使用无障碍包名观察且不申请投屏权限；Android 10 也可只记录活动。规则仍允许某些应用完整内容时，投屏模式需要正常系统授权。更新规则前需停止采集。已入队的记录保留入队时的字段与权限结果，不因后续设置改变而删除或重写。
...
无障碍路径在请求截图前后核对窗口信息，有变化则丢弃。无论是否配置排除，未知前台、多应用窗口、无障碍浮层及无法识别的窗口都暂停采样。键盘和系统辅助窗口即使非活动，也将其包名纳入同一分级规则；前台为 content 时，任一辅助窗口为 activity/off，整次内容采样暂停，不改存为活动；前台本已是 activity 时，只记录该前台应用，绝不读取辅助窗口内容。可见窗口只取类型与根节点包名，不读取键盘候选、剪贴板或控件文字。显式旧排除列表始终优先于新的应用分级规则。
...
来源配置和状态位于各安装环境独立的 `noBackupFilesDir/local-sources`，使用 Keystore 加密、临时文件 fsync 后原子替换。总缓存上限为 64 MiB 与首页队列上限的较小值，单来源最多 4096 个待发版本；达到上限暂停增加，不默删未确认数据。上传严格核对来源 ID、外部 ID、revision、记录 UUID 和重复标记，再移除对应待发项；丢 ACK 后重试同一版本。节点 URL 或令牌变化会重置该目标的确认状态并重新提交当前快照。文件附带提供者实际报告的非负大小和可取得的最后修改时间；SAF 不提供的创建、访问或元数据变更时间保持缺省。完整扫描发现文件消失时，删除版本保留最后已知大小/修改时间，并另记本次 `deletionObservedAt`；它不是文件真实删除时间。正文变化、删除、恢复形成不同的链式 revision；只有完整成功扫描才生成不带原文的删除版本，旧版本重试不会倒退中央当前指针。
