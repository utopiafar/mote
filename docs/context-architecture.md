...
| 产物 `context_artifacts` | 类型、成员、代表原文、时间范围、处理器与版本、配置指纹、修订、生成时间 | 输入变化、删除、来源被替代会使相关当前产物失效；历史保留和隐私删除按精确依赖处理 |
...
- 服务端 batch 最多 25 条、bundle 最多 500 条；客户端按自己的条数、字节与公平调度预算分包，保留逐条确认语义：外层事务配合成员 savepoint，合法项成功，冲突项单独返回错误。重试仍使用原 ID，删除墓碑防止旧队列复活资料。
