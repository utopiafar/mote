...
> Coding 接入使用 Cordis source pipeline：原始事件只存文件归档，按会话发布正式资料后才索引和送入模型。普通记录来源保留各自物理原件，但组织工作也经过持久执行引擎与正式资料发布。MVP 断代、统一收据和业务决策见 [后端流程重构工作稿](backend-plugin-rearchitecture.md) 与 [Source pipelines](source-pipelines.md)。
...
查询 Agent 可通过只读工具查看正式资料。原件、OCR、转写和文件片段仍是可核验的输入；查询可用当前已完成的部分资料并获知尚缺哪些产物。普通来源的正式资料发布会记录按来源配方声明的 Memory 依赖，所需产物就绪后再触发固定 revision 的 Memory 工作；Coding 会话也在资料发布和稳定后进入原有 Memory 执行器。模型不得因收据已返回而假设资料或 Memory 已完成。
...
需要新聚合边界的可信连接器可在 `init` 中调用 `ctx.materialOrganizers.registry.register(organizer)`。`organizer` 声明稳定 `id`、`version`，并实现按来源协议字段选择分组的 `select(record)`、组的稳定资料 ID `identity(group)` 和生成 `MaterialDraft` 的 `build(reader, group)`。受控 `reader` 只提供已固定范围的分页原件，不交出完整 Store 或物理目录。默认单条来源资料使用 `source-item` 组织槽；若新连接器要替换它，可声明 `slot: 'source-item'`、更高的整数 `priority`，并让 `select` 只匹配自己的 `sourceId`。若此形态必须排除其他槽的匹配组织器，还可声明 `exclusive: true`；同优先级的独占匹配会报错。`register` 返回注销函数，连接器应在 `close` 中调用；常规单条来源资料已由内置组织器覆盖，无需重复注册。新组织器安装或其 `version` 变化后，运行时会持久记录并分批重扫已有采集记录；资料目录在重扫完成前可能仍包含旧修订。
...
| Coding Agent 会话 | 依来源、provider、projectKey、sessionId 汇成一份会话资料；原始事件留在私有文件归档，后续追加只整理增量尾部 | 资料块保存稳定会话定位；原始事件不进入 Agent 目录。缺片和引用原件仍标记部分覆盖 |
| 连续屏幕观察 | 使用现有屏幕分组键形成片段，正文合并去重 OCR、应用和时间信息，仅保留少量确定性关键帧定位 | 每次观察的成员 ID 与采集时间仍在受控归档；高频单张截图默认不进入 Agent 目录，原图展开要经当前资料授权 |
...
普通查询模型先看到有界目录、检索命中、正式资料元数据和已发布记忆，再按需要读取正式资料的固定修订与内容范围。中央 Agent 提供 `material_catalog`、`material_read` 等只读入口；外部 MCP 读凭据提供 `mote_materials`、`mote_material`、`mote_material_read`，使用同一查询可见性规则，不枚举高频截图的全部原始成员。MCP 写凭据只可写允许的既有来源，不能读取这些资料工具。
