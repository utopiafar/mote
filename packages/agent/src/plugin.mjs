...
export async function apply(ctx) {
  ...
  const definitions = [
    ...
    ["memories","Progressive memory disclosure: omit id for short memory cards; pass a discovered id for its statement, uncertainty and original supporting evidence previews. These are model-derived proposals/published memories, never independent facts. Expand the provided original evidence ids before relying on a memory. Stale memories are excluded.",{...range,query:{type:'string',description:'Literal full-text search terms. Chinese and English are supported; reformulate when needed.'},tier:{type:'string',enum:['episode','consolidated']},kind:{type:'string',enum:['episodic','semantic','procedural']},id:{type:'string',description:'Memory id from a previous overview'}}],
    [
      "search_context",
