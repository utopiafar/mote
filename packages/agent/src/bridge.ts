...
import {fileEvidenceSchema,recordMetadataSchema, sourceMetadataSchema, sourceSchema} from '@mote/shared';
import type {
  ContextReader,
  ContextRecord,
  ContextRange,
  ...
  const server: Server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (tool === "devices") {
        ...
        if(Array.isArray(value))value=value.filter(r=>(!scope.deviceId||r.deviceId===scope.deviceId)&&(!scope.after||r.capturedAt>=scope.after)&&(!scope.before||r.capturedAt<scope.before));
