...
export const CONTEXT_TOOLS: [string,string,Record<string,Record<string,unknown>>][] = [
    ...
    ['material_catalog','Browse formal archived materials as bounded metadata only. Returns pinned revision refs, titles, source identity, time coverage and read costs; it does not expose bodies or original evidence IDs. Use material_read with an exact ref returned in this run to inspect a relevant page. Missing results do not prove absence of original captures.',{query:{type:'string',description:'Literal search in published material text; prefer this for Coding conversations.'},...range,sourceId:{type:'string',description:'Exact source ID, if known'},kind:{type:'string',description:'Exact registered material kind'},cursor:{type:'string',description:'Opaque cursor from the previous material_catalog page'}}],
    ...
    ['segments', 'Browse bounded processed activity segments after memory cards. These are exact-text reductions with original members and times, not inferred tasks or independent facts. Pass its returned ref as id to pin the segment revision; a legacy bare id selects its current version; use evidence on member ids to verify facts. Missing or pending segments never mean no original evidence exists: use search_context for recent or exact facts.', {...contextFilters,id:{type:'string'},query:{type:'string'},cursor:{type:'string'}}],
