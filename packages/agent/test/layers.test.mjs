...
test('file chunks require prior discovery, preserve immutable timed citations and cannot broaden device scope',async t=>{
 ...
 const answer=parseAnswer(JSON.stringify({answer:`A reported plan [${chunk.id}]`,citationIds:[chunk.id]}),b.records);
 assert.deepEqual(answer.citations[0].fileEvidence,chunk.fileEvidence);
});

test('a selected file chunk keeps its timing and exact text bounds while file discovery remains unavailable inside extraction',async t=>{
 const parentId='22222222-2222-4222-8222-222222222222',id='33333333-3333-4333-8333-333333333333';
 const chunk={...record,id,ocrText:'HIDDEN_PREFIX生成的转写片段HIDDEN_SUFFIX',provenance:{sourceId:'fixture',revision:'v1',layer:'derived'},fileEvidence:{captureId:parentId,revision:'v1',artifactId:'44444444-4444-4444-8444-444444444444',chunkId:id,startMs:1200,endMs:1800}};
 const text='生成的转写片段',offset=chunk.ocrText.indexOf(text);
 const b=await startBridge(reader({evidence:async()=>[chunk]}),{question:'Extract fixture',evidenceIds:[id],evidenceRanges:[{id,offset,length:text.length}]},8);t.after(()=>b.close());
 assert.equal(b.seedEvidence[0].ocrText,text);assert.deepEqual(b.seedEvidence[0].fileEvidence,chunk.fileEvidence);
 assert.equal((await request(b,'file_chunks',{id:parentId})).status,400);
 assert.equal((await request(b,'evidence',{ids:[id],offset:0,length:2})).status,400);
 assert.equal((await request(b,'evidence',{ids:[id]})).body.data[0].ocrText,text);
