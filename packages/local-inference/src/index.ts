...
export function parseVisionDecision(raw: string): VisionDecision {
  if (typeof raw !== 'string' || raw.length > 16000) throw new Error('本地视觉模型输出过大或无效');
  const v: unknown = JSON.parse(raw.trim());
  if (!v || typeof v !== 'object' || Array.isArray(v)) throw new Error('本地视觉模型必须返回 JSON 决策');
  const r = v as Record<string, unknown>;
  if (typeof r.allow !== 'boolean' || Object.keys(r).some(k => !['allow', 'reason', 'labels'].includes(k))) throw new Error('本地视觉模型决策格式无效');
  if (r.reason !== undefined && (typeof r.reason !== 'string' || r.reason.length > 240)) throw new Error('本地视觉模型原因格式无效');
  if (r.labels !== undefined && (!Array.isArray(r.labels) || r.labels.length > 12 || r.labels.some(x => typeof x !== 'string' || x.length > 64))) throw new Error('本地视觉模型标签格式无效');
...
export function validateModelUrl(value: string): string {
  ...
  if (u.protocol !== 'https:' || u.username || u.password || u.hash) throw new Error('模型下载需要不含账号及片段的 HTTPS 文件地址');
  return u.toString();
}
function validateManifest(m: ModelManifest): void {
  if (!/^[a-zA-Z0-9._-]+$/.test(m.fileName) || m.fileName === '.' || m.fileName === '..' || !/^[a-f0-9]{64}$/.test(m.sha256) || !Number.isSafeInteger(m.size) || m.size <= 0) throw new Error('模型清单无效');
...
async function fetchModel(url: string, headers: Record<string, string>, signal: AbortSignal, fetcher: typeof fetch): Promise<Response> {
  for (let redirects = 0; redirects <= 8; redirects++) {
    ...
    if (!location) throw new Error('模型下载重定向缺少地址');
...
export class ModelStore {
  ...
  async verifiedPath(): Promise<string> {
    if (!await verifyModelFile(this.path, this.manifest)) throw new Error('千问视觉模型缺失或校验失败，请先下载或导入模型；截图已跳过');
    return this.path;
  }
  private async exclusive<T>(action: () => Promise<T>): Promise<T> {
    if (activeDirectories.has(this.directory)) throw new Error('模型正在下载或导入，请等待或取消当前操作');
  ...
  async importFile(source: string): Promise<string> {
    return this.exclusive(async () => {
      ...
      try {
        if (await bytesAt(source) !== this.manifest.size) throw new Error('导入文件大小与固定模型不符');
        const out = await open(temporary, 'wx', 0o600);
        let total = 0;
        try {
          for await (const chunk of createReadStream(source)) {
            total += chunk.length;
            if (total > this.manifest.size) throw new Error('导入文件过大');
            await out.writeFile(chunk);
          }
          await out.sync();
        } finally { await out.close(); }
        if (!await verifyModelFile(temporary, this.manifest)) throw new Error('导入模型 SHA-256 校验失败');
  ...
  async download(options: DownloadOptions): Promise<string> {
    return this.exclusive(async () => {
      ...
      if (!sources.every(s => ['mirror', 'official', 'custom'].includes(s))) throw new Error('模型下载来源无效');
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
          ...
          try {
            ...
            if (!await verifyModelFile(this.partialPath, this.manifest)) {
              await unlink(this.partialPath); throw new Error('SHA-256 校验失败');
            }
            options.signal?.throwIfAborted();
      ...
      throw new Error('模型下载或校验失败；已保留可续传部分，请重试、更换来源或离线导入');
      } finally {
  ...
  private async downloadOnce(url: string, source: string, options: DownloadOptions): Promise<void> {
    ...
    let timer = setTimeout(() => controller.abort(new Error('模型下载连接超时')), 45000);
    const touch = () => { clearTimeout(timer); timer = setTimeout(() => controller.abort(new Error('模型下载读取超时')), 45000); };
    ...
    try {
      ...
      if (response.status === 206) {
        ...
        if (!match || Number(match[1]) !== offset || Number(match[3]) !== m.size || Number(match[2]) !== m.size - 1) throw new Error('模型下载续传范围无效');
      } else if (response.status === 200) offset = 0;
      else if (response.status === 416) { await unlink(partial).catch(() => undefined); throw new Error('模型下载范围已重置'); }
      else throw new Error('模型下载请求失败');
      if (!response.body) throw new Error('模型下载内容为空');
      ...
      try {
        ...
        try {
          while (true) {
            ...
            if (received + value.length > m.size) throw new Error('模型下载响应超过清单大小');
      ...
      if (received !== m.size) throw new Error('模型文件尚未下载完整');
...
export class VisionModelStore {
  ...
  constructor(private readonly directory: string, private readonly manifest: VisionManifest = QWEN_MODEL, fetcher: typeof fetch = fetch, retryDelayMs = 1000) {
    if (manifest.files.length !== 2 || manifest.files.filter(f => f.role === 'language').length !== 1 || manifest.files.filter(f => f.role === 'vision').length !== 1 || manifest.totalBytes !== manifest.files.reduce((n, f) => n + f.size, 0)) throw new Error('视觉模型清单无效');
  ...
  async download(options: DownloadOptions): Promise<VisionPaths> {
    ...
    for (const [index, file] of this.files.entries()) {
      ...
      if (options.source === 'custom') {
        ...
        if (base.search) throw new Error('自定义模型来源需要 HTTPS 目录，不含查询参数');
  ...
  async importFiles(paths: string[]): Promise<void> {
    if (!Array.isArray(paths) || paths.length < 1 || paths.length > 2 || paths.some(p => typeof p !== 'string')) throw new Error('请选择语言模型与视觉模型文件，最多两个');
    ...
    for (const path of paths) {
      ...
      if (index < 0 || matched.some(f => f.index === index)) throw new Error('模型文件与固定千问版本的大小或 SHA-256 不符，或重复选择');
