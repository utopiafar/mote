...
RUN test -x /bin/bash && node --input-type=module -e "import {accessSync} from 'node:fs'; import {createRequire} from 'node:module'; import {bundledSkills} from './packages/agent/dist/skills.js'; accessSync('./apps/server/dist/import-parser.mjs'); if(bundledSkills.length!==3 || typeof createRequire(import.meta.url)('node-pty').spawn!=='function') throw Error('Import runtime is incomplete');"
