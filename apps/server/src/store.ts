...
export class Store {
  ...
  logicalBytes() {
    ...
    const jsonTables=['captures','memories','source_connections','conversations','memory_jobs','memory_batches','archived_files','import_jobs','file_artifacts','file_reviews','insight_runs','action_meta','action_proposals','action_targets'].filter(name=>tables.has(name));
