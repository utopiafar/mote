...
export const defaultLifecycleSettings:LifecycleSettings={
  ...
  batchCharacters:12000,recentTurns:8,contextCharacters:24000,summaryCharacters:6000,
};
...
type State={cursor:number;lastSuccess:number;retryAt?:number;failures:number;active?:LifecycleWindow;lastRun?:{id:string;through:number;completedAt:number};error?:string};
...
export class MemoryLifecycle {
  ...
  view(){const settings=this.settings();return {settings,trigger:'interval AND incremental',storage:'text',extensions:[...this.extensions.values()].map(e=>{
    const state=this.state(e.id),p=settings[e.id],pendingChanges=this.count(e,state.cursor),dueAt=state.lastSuccess+p.intervalHours*3600000;
    return {id:e.id,version:e.version,stream:e.stream,pendingChanges,dueAt,retryAt:state.retryAt,cursor:state.cursor,failures:state.failures,error:state.error,
      status:!p.enabled?'disabled':state.active?'pending':!this.configured()?'waiting_for_model':this.now()<dueAt?'waiting_for_interval':pendingChanges<p.minChanges?'waiting_for_increment':'ready',
  ...
  private async execute(){
    for(const extension of this.extensions.values()){
      ...
      if(!state.active){
        ...
        const events=this.events(extension,state.cursor,p.maxItems);if(!events.length)continue;
      ...
      try{
        ...
        state.cursor=state.active.through;state.lastSuccess=this.now();state.lastRun={id:state.active.id,through:state.cursor,completedAt:state.lastSuccess};delete state.active;delete state.error;delete state.retryAt;state.failures=0;
