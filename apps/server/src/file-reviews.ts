...
export class FileReviews {
  ...
  confirm(id:string,reviewId:string,raw:unknown){
    ...
    db.exec('BEGIN IMMEDIATE');try{
      ...
      if(transcript){db.prepare("UPDATE file_reviews SET status='stale' WHERE capture_id=? AND status='proposed'").run(id);db.exec("DELETE FROM insights; UPDATE memories SET json=json_set(json,'$.status','stale')");this.files.store.invalidateConversationAnswers();}
