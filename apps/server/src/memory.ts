  ...
  extract(result:QueryResult,model:string,options:MemoryExtractOptions={}) {
    ...
    try{
      ...
      const validateClaims=()=>parsed.data.memories.map((m,candidateIndex)=>{
        ...
        if((domain==='coding')!==!!m.coding)throw new MemoryOutputValidationError('schema','Only coding claims require coding applicability; personal claims must omit coding');
