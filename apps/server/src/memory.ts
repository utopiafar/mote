...
export class MemoryStore {
  constructor(public store:Store,private readEvidence:(ids:string[])=>{id:string;deviceId?:string;capturedAt:string}[]=ids=>store.evidence(ids)){}
  ...
  extract(result:QueryResult,model:string) {
    ...
    // Some providers repeat the outer citation envelope inside the JSON answer.
    // Accept only that exact redundant envelope, with the same verified IDs.
    ...
    if(parsed.data.citationIds){const repeated=new Set(parsed.data.citationIds);if(repeated.size!==allowed.size||[...repeated].some(id=>!allowed.has(id)))throw new StoreError('Repeated memory citation envelope does not match verified evidence',502);}
    for(const m of parsed.data.memories)for(const id of m.evidenceIds)if(!allowed.has(id)||!this.readEvidence([id]).length)throw new StoreError('Memory evidence is missing or was not retrieved',409);
