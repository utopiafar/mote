...
export type MemoryExtractOptions={tier?:Memory['tier'];relatedMemoryIds?:string[];skillVersion?:string;evidenceRanges?:EvidenceRange[];expectedFingerprints?:Record<string,string>;onSaved?:(items:Memory[])=>void};
export class MemoryStore {
  ...
  extract(result:QueryResult,model:string,options:MemoryExtractOptions={}) {
    ...
    try{
      ...
      for(const m of claims){const fingerprint=sha256(JSON.stringify([options.tier??'episode',m.statement,[...m.evidenceIds].sort(),m.evidence.map(e=>[e.id,e.contentHash,e.offset,e.length])]));
