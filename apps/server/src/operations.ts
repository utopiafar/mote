...
const summary=(row:Record<string,unknown>):OperationSummary=>({id:String(row.id),kind:String(row.kind)||'processing',state:row.state as OperationState,total:Number(row.total),notScheduled:Number(row.not_scheduled),optionalIssues:Number(row.total)-Number(row.not_scheduled)-states.reduce((sum,state)=>sum+Number(row[state]),0),counts:Object.fromEntries(states.map(state=>[state,Number(row[state])])) as OperationSummary['counts'],createdAt:Number(row.created_at),updatedAt:Number(row.updated_at)});
...
export class Operations {
 ...
 detail(id:string,cursor=0,limit=50):OperationDetail{
  ...
  return {operation:summary(row),steps:rows.slice(0,limit).map(step=>({id:String(step.id),kind:String(step.kind).startsWith('file-step.')?'file-step':String(step.kind).startsWith('query.run.')?'query.run':String(step.kind).startsWith('insight.run.')?'insight.run':String(step.kind).startsWith('lifecycle-window.')?'lifecycle-window':String(step.kind).startsWith('embedding.query.')?'embedding.query':String(step.kind),state:step.state as OperationState,attempts:Number(step.attempts),current:Boolean(step.active),...(step.optional?{optional:true}:{}),notScheduled:Boolean(step.optional)&&step.state==='blocked',availableAt:Number(step.available_at),updatedAt:Number(step.updated_at),...(step.error?{reason:/^[a-z][a-z0-9_]{0,80}$/.test(String(step.error))?String(step.error):'processing_failed'}:{}),dependencies:this.store.db.prepare('SELECT dependency_id FROM execution_dependencies WHERE step_id=? ORDER BY dependency_id LIMIT 100').all(step.id).map(row=>String(row.dependency_id))})),nextCursor:rows.length>limit?Number(rows[limit-1].rowid):null};
 }
