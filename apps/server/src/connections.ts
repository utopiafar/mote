...
const denied=()=>new ConnectionError('connection_scope_denied',403,moteText("此连接仅可同步本设备资料；中央管理和浏览需要单独使用所有者令牌。"));
const digest=(value:string)=>createHash('sha256').update(value).digest('hex');
function serverUrl(value:unknown){
  if(typeof value!=='string')throw new ConnectionError('connection_url_invalid',400,moteText("请输入完整的 HTTPS 中央节点地址；本机回环地址可使用 HTTP。"));
  try{return connectionServerUrl(value);}catch{throw new ConnectionError('connection_url_invalid',400,moteText("请输入完整的 HTTPS 中央节点地址；地址不能包含账号、路径、查询或片段。"));}
}
export class Connections {
  ...
  private async persist(credentials:ConnectionCredential[]){
    ...
    if(credentials.length>500||Buffer.byteLength(JSON.stringify({version:1,credentials:reserved}))>2*1024*1024)throw new ConnectionError('connection_limit',409,moteText("连接记录已达到数量或存储上限，请联系节点管理员。"));
    try{await this.file.write({version:1,credentials});}catch{throw new ConnectionError('connection_storage_failed',503,moteText("连接凭据未保存，操作未生效；请检查中央节点存储后重试。"));}
  ...
  redeem(raw:unknown){
    ...
    return this.serialize(async()=>{
      ...
      if(!invitation)throw new ConnectionError('invitation_invalid_or_expired',410,moteText("邀请已使用、取消或过期，请在中央节点重新生成。"));
      if(invitation.authorizedDeviceId&&invitation.authorizedDeviceId!==input.deviceId)throw new ConnectionError('invitation_device_mismatch',403,moteText("此邀请已绑定另一台设备，请在中央节点确认设备后重新生成。"));
  ...
  mintMcp(raw:unknown,config?:ConnectorConfig){
    ...
    if(!config?.mcpEnabled)throw new ConnectionError('mcp_disabled',409,moteText("请先配置 MOTE_MCP_READ_TOKEN 并开启 MOTE_MCP_ENABLED，然后重启节点。"));
    if(input.access==='write'&&(!config.mcpWriteEnabled||!config.mcpWriteSourceIds?.length))throw new ConnectionError('mcp_write_disabled',409,moteText("请先配置 MOTE_MCP_WRITE_TOKEN、MOTE_MCP_WRITE_SOURCE_IDS，并开启 MOTE_MCP_WRITE_ENABLED 后重启节点。"));
  ...
  assertCollectorRoute(credential:ConnectionCredential,method:string,route:string){
    ...
    const permitted:Record<string,string[]>={'/api/actions':['GET','HEAD'],'/api/actions/deliveries':['GET','HEAD'],'/api/actions/targets':['POST'],'/api/actions/:id/confirm':['POST'],'/api/actions/:id/dismiss':['POST'],'/api/actions/:id/claim':['POST'],'/api/actions/:id/receipt':['POST'],'/api/file-sync/v1/head':['GET','HEAD'],'/api/file-sync/v1/capabilities':['GET','HEAD'],'/api/file-sync/v1/uploads':['POST'],'/api/file-sync/v1/uploads/:id':['GET','HEAD'],'/api/file-sync/v1/uploads/:id/parts/:part':['PUT'],'/api/file-sync/v1/uploads/:id/commit':['POST'],'/api/file-sync/v1/revisions':['PUT'],'/api/files':['GET','HEAD'],'/api/files/:id':['GET','HEAD'],'/api/files/:id/chunks':['GET','HEAD'],'/api/files/:id/content':['GET','HEAD'],'/api/files/:id/playback':['POST'],'/api/captures':['POST'],'/api/captures/batch':['POST'],'/api/capture-browser/reconcile':['POST'],'/api/capture-browser':['GET','HEAD'],'/api/capture-browser/sessions':['GET','HEAD'],'/api/capture-browser/albums':['GET','HEAD'],'/api/capture-browser/album-images':['GET','HEAD'],'/api/capture-browser/:id':['GET','HEAD'],'/api/capture-browser/:id/image':['GET','HEAD'],'/api/capture-browser/:id/ocr':['POST'],'/api/media-activity':['GET','HEAD'],'/api/notes':['POST'],'/api/devices/heartbeat':['POST'],'/api/sources':['GET','HEAD','POST'],'/api/sources/:id':['PATCH'],'/api/sources/:id/items':['GET','HEAD','PUT'],'/api/sources/:id/item':['GET','HEAD'],'/api/sources/:id/history':['GET','HEAD'],'/api/source-items':['GET','HEAD']};
  ...
  assertCapture(c:ConnectionCredential,body:unknown){
    ...
    this.assertPlatform(c,input.platform);if(input.provenance!==undefined||!['screen','note','activity','media','notification','device_event'].includes(String(input.source)))throw denied();
