...
export type FormatTask={kind:'text';path:string;maxBytes:number;chunkCharacters:number}|{kind:'zip';path:string;output:string;maxFiles:number;maxBytes:number}|{kind:'plain';inputs:{file:ArchivedFile;path:string}[];createdAt:string;manifest:string};
type Results={text:{durationMs:number;segments:{startMs:number;endMs:number;text:string}[]};zip:{files:{name:string;path:string;bytes:number}[];bytes:number};plain:{count:number;samples:{title:string;text:string;kind:string;attachmentCount:number}[];hash:string}};
...
export async function formatWork<K extends FormatTask['kind']>(task:Extract<FormatTask,{kind:K}>,signal?:AbortSignal):Promise<Results[K]>{
 ...
 try{return await new Promise<Results[K]>((resolve,reject)=>{
  ...
  const done=(error?:unknown,result?:Results[K])=>{if(settled)return;settled=true;deadline.removeEventListener('abort',abort);if(error){child.kill('SIGKILL');reject(error);}else resolve(result!);};
  ...
  child.once('message',(message:any)=>message.ok?done(undefined,message.result):done(new StoreError('Format decoding failed: '+String(message.code),422)));
  child.send(task,error=>{if(error)done(error);});
 });}finally{active--;queue.shift()?.();}
}

export async function extractUtf8(body:AsyncIterable<Buffer>,sizeBytes:number,signal:AbortSignal){
 if(sizeBytes>2*1024*1024)throw new StoreError('Text exceeds extraction limit',413);
 ...
 try{
  ...
  return await formatWork({kind:'text',path,maxBytes:2*1024*1024,chunkCharacters:4000},signal);
