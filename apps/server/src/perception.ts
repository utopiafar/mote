...
export const perceptionSettingsSchema=z.object({
  ocrProcessorId:z.string().max(100).default('image.http'),semanticProcessorId:z.string().max(100).default('image.http'),concurrency:z.number().int().min(1).max(8).default(1),
...
export class Perception {
  ...
  constructor(private store:Store,private runtime:FileProcessorRuntime,engine?:ExecutionEngine,private mediaAssets?:MediaAssets,private probeOcr:()=>Promise<boolean>=managedOcrReady){
    ...
    if(mediaAssets&&!store.db.prepare("SELECT 1 FROM settings WHERE key='managed-ocr-v1'").get()){
      ...
      const next=perceptionSettingsSchema.parse({...prior,enabled:prior.enabled??true,ocrProcessorId:prior.ocrProcessorId??'image.http',ocrEndpoint:prior.ocrEndpoint||managedOcrEndpoint(),concurrency:prior.concurrency??1});
  ...
  private execution(settings:PerceptionSettings,kind:string){return sha256(JSON.stringify([settings.enabled,settings.allowExternalProcessing,settings.providerRevision,kind==='ocr'?settings.ocrProcessorId:settings.semanticProcessorId,kind==='ocr'?settings.ocrEndpoint:settings.semanticEndpoint,kind==='ocr'&&settings.ocrEndpoint===managedOcrEndpoint()&&this.mediaAssets?MEDIA_CATALOG.ocr.version:'']));}
  ...
  view(){return {recent:this.store.db.prepare('SELECT capture_id AS id,kind,state,error,auto_eligible AS autoEligible FROM perception_jobs ORDER BY created_at DESC LIMIT 40').all(),settings:this.settings(),jobs:this.store.db.prepare('SELECT kind,state,auto_eligible AS autoEligible,count(*) AS count FROM perception_jobs GROUP BY kind,state,auto_eligible').all()};}
  ...
  retry(id:string,kind:'ocr'|'semantic'){
    ...
    this.store.db.prepare("INSERT INTO perception_jobs(capture_id,kind,state,created_at,auto_eligible) VALUES(?,?,'waiting',?,1) ON CONFLICT(capture_id,kind) DO UPDATE SET state='waiting',attempts=0,available_at=0,requested=1,auto_eligible=1").run(id,kind,Date.now());
  ...
  private async process(step:ExecutionStep,signal:AbortSignal){
    ...
    const fingerprint=sha256(JSON.stringify([step.input.blobHash,processor.id,processor.version,kind,url,settings.providerRevision,kind==='ocr'&&url===managedOcrEndpoint()&&this.mediaAssets?MEDIA_CATALOG.ocr.version:'',...(kind==='semantic'?[id,record.capturedAt,record.deviceId,record.appId,record.windowTitle,record.provenance]:[])]));
