...
export function installOperationProjection(store:Store){
 const db=store.db;if(db.prepare("SELECT 1 FROM settings WHERE key='operation-projection-v1'").get()){installOperationHierarchy(store);installOptionalTerminalProjection(store);return;}
 ...
 try{
  ...
  db.exec('COMMIT');installOperationHierarchy(store);installOptionalTerminalProjection(store);
 }catch(error){if(db.isTransaction)db.exec('ROLLBACK');throw error;}
}

/** Optional work retains its real failure receipt without changing a completed
 * required task into failure. Counters remain an incremental read projection. */
function installOptionalTerminalProjection(store:Store){
 const db=store.db;if(db.prepare("SELECT 1 FROM settings WHERE key='operation-optional-terminal-v2'").get())return;
 ...
 db.exec('BEGIN IMMEDIATE');
 try{
  db.exec(`DROP TRIGGER operation_link_added;DROP TRIGGER operation_link_changed;DROP TRIGGER operation_link_removed;DROP TRIGGER operation_step_changed;
   CREATE TRIGGER operation_link_added AFTER INSERT ON execution_operation_steps BEGIN
    INSERT OR IGNORE INTO operation_progress(id,created_at,updated_at) SELECT new.operation_id,created_at,updated_at FROM execution_steps WHERE id=new.step_id;
    ${applyLink('new','+')}
   END;
   ...
   CREATE TRIGGER operation_link_removed BEFORE DELETE ON execution_operation_steps BEGIN ${applyLink('old','-')} END;
   CREATE TRIGGER operation_step_changed AFTER UPDATE OF state,updated_at ON execution_steps BEGIN ${applyState('old','-')} ${applyState('new','+')} END;
   UPDATE operation_progress SET total=p.total,not_scheduled=p.not_scheduled,${states.map(state=>`${state}=p.${state}`).join(',')}
    FROM (SELECT o.operation_id,sum(o.active) total,sum(o.active AND o.optional AND e.state='blocked') not_scheduled,${states.map(state=>`sum(o.active AND e.state='${state}' AND NOT ${excluded('e.state','o.optional')}) ${state}`).join(',')} FROM execution_operation_steps o JOIN execution_steps e ON e.id=o.step_id GROUP BY o.operation_id) p WHERE id=p.operation_id;
   INSERT OR IGNORE INTO settings VALUES('operation-optional-terminal-v2','1');`);
  db.exec('COMMIT');
 }catch(error){if(db.isTransaction)db.exec('ROLLBACK');throw error;}
}
