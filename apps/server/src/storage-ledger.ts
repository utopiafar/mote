...
export class StorageLedger {
  ...
  private refresh(){
    ...
    Object.assign(expressions,{material_block_payloads:'length(CAST(text AS BLOB))',material_revisions:'length(CAST(manifest AS BLOB))+256',material_blocks:'256',material_members:'256',material_evidence:'128',source_archive_sizes:'bytes',run_execution_owners:'128',operation_parents:'length(CAST(parent_id AS BLOB))+length(CAST(child_id AS BLOB))+256',file_configuration_snapshots:'length(CAST(receipt AS BLOB))+128',file_configuration_aliases:'256',provider_cooldowns:'length(CAST(code AS BLOB))+128',operation_progress:'length(CAST(id AS BLOB))+256',operation_changes:'length(CAST(operation_id AS BLOB))+32',execution_steps:'length(CAST(input AS BLOB))+512',execution_resources:'length(CAST(resource_key AS BLOB))+64',import_upload_parts:'bytes',assets:'bytes',blobs:'bytes',file_blobs:'bytes',file_objects:'bytes',file_chunks:'length(CAST(text AS BLOB))+COALESCE(length(embedding),0)',file_versions:'length(CAST(manifest AS BLOB))',file_uploads:'length(CAST(manifest AS BLOB))',file_parts:'bytes'});
