...
export function createMoteMcp(ctx:ConnectorContext,write=false,track?:<T>(work:Promise<T>)=>Promise<T>,authorize?:()=>void) {
  ...
  const evidence=(r:CaptureRecord,offset=0,length=2000)=>{let start=Math.min(offset,r.ocrText.length),end=Math.min(start+length,r.ocrText.length);const split=(at:number)=>at>0&&at<r.ocrText.length&&/[\uD800-\uDBFF]/.test(r.ocrText[at-1])&&/[\uDC00-\uDFFF]/.test(r.ocrText[at]);if(split(start))start--;if(split(end))end--;if(end<=start&&start<r.ocrText.length)end=Math.min(start+2,r.ocrText.length);return {id:r.id,capturedAt:r.capturedAt,appName:r.appName,text:r.ocrText.slice(start,end),textRange:{offset:start,total:r.ocrText.length,nextOffset:end<r.ocrText.length?end:null},source:r.source,provenance:r.provenance,mood:r.mood,appId:r.appId,deviceId:r.deviceId,durationMs:r.durationMs,receivedAt:r.receivedAt,privacy:r.privacy,metadata:r.metadata};};
