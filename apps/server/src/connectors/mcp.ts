...
export function createMoteMcp(ctx:ConnectorContext,write=false,track?:<T>(work:Promise<T>)=>Promise<T>,authorize?:()=>void) {
  ...
  server.registerTool('mote_search',{description:'Generic lexical archive search. The calling model selects search text; Mote performs no keyword-based intent classification.',inputSchema:{...range,query:z.string().max(2000)},annotations:readonly},async args=>safe(()=>[...(ctx.files?.search(args)??[]),...ctx.store.search(args)].slice(0,args.limit).map(r=>evidence(r as CaptureRecord))));
