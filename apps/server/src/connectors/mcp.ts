...
export function createMoteMcp(ctx:ConnectorContext,write=false,track?:<T>(work:Promise<T>)=>Promise<T>,authorize?:()=>void) {
  ...
  const evidence=(r:CaptureRecord,offset=0,length=2000)=>{let start=Math.min(offset,r.ocrText.length),end=Math.min(start+length,r.ocrText.length);const split=(at:number)=>at>0&&at<r.ocrText.length&&/[\uD800-\uDBFF]/.test(r.ocrText[at-1])&&/[\uDC00-\uDFFF]/.test(r.ocrText[at]);if(split(start))start--;if(split(end))end--;if(end<=start&&start<r.ocrText.length)end=Math.min(start+2,r.ocrText.length);return {id:r.id,capturedAt:r.capturedAt,appName:r.appName,text:r.ocrText.slice(start,end),textRange:{offset:start,total:r.ocrText.length,nextOffset:end<r.ocrText.length?end:null},source:r.source,provenance:r.provenance,mood:r.mood};};
  const range={after:z.string().datetime({offset:true}).optional(),before:z.string().datetime({offset:true}).optional(),deviceId:z.string().max(128).optional(),limit:z.number().int().min(1).max(100).default(30)};
  ...
  server.registerTool('mote_timeline',{description:'Read the archive including screenshots, authored notes and current source revisions. Results contain bounded excerpts and pagination.',inputSchema:{...range,cursor:z.string().max(4096).optional()},annotations:readonly},async args=>safe(()=>{const page=ctx.store.list(args);return {...page,items:page.items.map(r=>evidence(r))};}));
  server.registerTool('mote_search',{description:'Generic lexical archive search. The calling model selects search text; Mote performs no keyword-based intent classification.',inputSchema:{...range,query:z.string().max(2000)},annotations:readonly},async args=>safe(()=>ctx.store.search(args).map(r=>evidence(r))));
  server.registerTool('mote_activity',{description:'Measured screen sample intervals only. Calendar appointments and authored notes do not establish time spent or completed work.',inputSchema:range,annotations:readonly},async args=>safe(()=>ctx.store.activity(args)));
