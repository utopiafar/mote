...
export function createMoteMcp(ctx:ConnectorContext,write=false,track?:<T>(work:Promise<T>)=>Promise<T>,authorize?:()=>void) {
  ...
  const contextFilter={...range,sourceId:z.string().max(128).optional(),query:z.string().max(2000).optional(),projectKey:z.string().max(200).optional(),repositoryKey:z.string().regex(/^[a-f0-9]{64}$/).optional(),provider:z.enum(['claude','codex','kimi']).optional(),sessionId:z.string().max(500).optional(),cursor:z.string().max(4096).optional()};
