...
export function contextIndex(store:Store,memories:Pick<MemoryStore,'page'>,sources:SourceStore,args:Range&{path?:string;query?:string}={}){
 ...
 if(path==='/context'||path==='/context/memory')for(const item of memories.page({...args,limit}).items)entries.push({layer:'memory',id:item.id,title:item.title,revision:item.revision,status:item.status,evidenceCount:item.evidenceCount,estimatedCharacters:6000,expand:'memories'});
 if(path==='/context'||path==='/context/episodes')for(const item of store.archive.page({...args,limit,maxCharacters:6000}).items)if(item)entries.push({layer:item.kind,id:item.id,revision:item.revision,preview:item.text,firstAt:item.firstAt,lastAt:item.lastAt,deviceId:item.deviceId,coverage:item.metadata.complete===true?'bounded_complete':'partial',estimatedCharacters:item.metadata.characters??12000,expand:'segments'});
