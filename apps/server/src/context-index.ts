...
export function contextIndex(store:Store,memories:Pick<MemoryStore,'page'>,sources:SourceStore,args:Range&{path?:string;query?:string}={},segments:Store['archive']['page']=args=>store.archive.page(args)){
 ...
 if(path==='/context'||path==='/context/memory')for(const item of memories.page({...args,layer:'memory',limit}).items)entries.push({layer:'memory',ref:formatEvidenceRef('memory',item.id),id:item.id,title:item.title,revision:item.revision,status:item.status,evidenceCount:item.evidenceCount,estimatedCharacters:6000,expand:'memories'});
