...
export class QueryRuns {
  ...
  cancel(id:string){const run=this.get(id);if(run.status!=='running')return run;this.controllers.get(id)?.abort();return this.get(id);}
  ...
  start(id:string,input:unknown,work:(observe:(event:AgentProgress)=>void,signal:AbortSignal)=>Promise<{conversationId:string;turnId:string}>){
    ...
    controller.signal.addEventListener('abort',()=>{run.status='cancelled';run.updatedAt=new Date().toISOString();this.save(run);},{once:true});
    ...
    const task=Promise.resolve().then(()=>work(observe,controller.signal)).then(result=>{if(controller.signal.aborted)return;Object.assign(run,{conversationId:result.conversationId,turnId:result.turnId,status:'completed'});}).catch(e=>{if(controller.signal.aborted)return;const saved=(e&&typeof e==='object'?(e as {conversation?:{conversationId:string;turnId:string}}).conversation:undefined);if(saved)Object.assign(run,saved);const safe=safeError(e);run.status='failed';run.error={code:safe.category,message:safe.message};}).finally(()=>{this.controllers.delete(id);if(run.evidenceRevision!==this.store.deletionRevision())run.events=run.events.map(({message:_,...e})=>e);run.updatedAt=new Date().toISOString();this.save(run);});
