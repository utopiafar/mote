...
export class QueryRuns {
  ...
  list():QueryRun[]{return (this.store.db.prepare("SELECT json FROM query_runs ORDER BY json_extract(json,'$.createdAt') DESC LIMIT 100").all() as {json:string}[]).map(r=>normalizeRun(JSON.parse(r.json)));}
  get(id:string):QueryRun{const row=this.store.db.prepare('SELECT json FROM query_runs WHERE id=?').get(id) as {json:string}|undefined;if(!row)throw new StoreError('Query run not found',404);return normalizeRun(JSON.parse(row.json));}
  start(id:string,input:unknown,work:(observe:(event:AgentProgress)=>void,signal:AbortSignal)=>Promise<{conversationId:string;turnId:string}>){
    ...
    controller.signal.addEventListener('abort',()=>{run.status='cancelled';run.execution={status:'cancelled',attempts:1,allowedActions:[]};run.updatedAt=new Date().toISOString();this.save(run);},{once:true});
