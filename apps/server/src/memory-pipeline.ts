...
export type MemoryPipelineQuery={question:string;skill:'memory-extraction'|'coding-memory';evidenceIds:string[];evidenceRanges:EvidenceRange[];timeZone?:string};
...
export class MemoryPipeline {
  ...
  create(raw:{evidenceIds:string[];importJobId?:string;originKey?:string;timeZone?:string}):MemoryJobDetail {
    if(this.closed)throw new StoreError('Memory pipeline is closed',503);
    const input=z.object({evidenceIds:z.array(z.string().uuid()).min(1).max(20000),importJobId:z.string().max(200).optional(),originKey:z.string().max(200).optional(),timeZone:z.string().max(100).refine(value=>{try{new Intl.DateTimeFormat('en',{timeZone:value});return true;}catch{return false;}},'Invalid time zone').optional()}).strict().parse(raw);
    ...
    for(const chunk of all){if(group.length&&(group[0].group!==chunk.group||characters+chunk.length>this.budget||group.length>=20)){groups.push(group);group=[];characters=0;}group.push(chunk);characters+=chunk.length;}
    ...
    for(const original of this.batches(id)){
      ...
      try{
        ...
        for(let generation=0;generation<2;generation++){
          ...
          const result=await this.options.query({question,skill:profile.skill,evidenceIds:[...new Set(chunks.map(c=>c.id))],evidenceRanges:ranges.map(range=>({...range})),timeZone:job.timeZone});
