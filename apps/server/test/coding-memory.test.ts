...
test('consolidation retains coding contracts and checkpoints each domain across a failed mixed window',async t=>{
 ...
 now=24*3600000;await lifecycle.tick();assert.deepEqual(calls,['personal','coding']);assert.equal(memories.page({tier:'consolidated'}).items.length,1);
 failCoding=false;now+=121000;await lifecycle.tick();assert.deepEqual(calls,['personal','coding','coding'],'completed personal generation is not repeated');
