...
test('10,000 raw events remain file-only; complete long document is indexed and cited by material sections',async t=>{
  const {store,materials,runtime,sources,reader}=await fixture(t);
  for(let offset=0;offset<10000;offset+=500)await sources.upsertBatch('coding',Array.from({length:500},(_,i)=>item(offset+i,'session-a',`Unique generated event ${offset+i}. `+'abcd '.repeat(90))));
  ...
  assert.equal(materials.list().items.length,0);runtime.tick();
  const material=materials.list({query:'Unique generated event'}).items[0];assert.ok(material);assert.equal(material.coverage.state,'complete');assert.ok(material.textLength>4_000_000);
  ...
  assert.ok(text.includes('Unique generated event 0.'));assert.ok(text.includes('Unique generated event 9999.'));assert.equal(materials.list().items.length,1);
  ...
  assert.equal(store.db.prepare('SELECT count(*) n FROM material_fts').get()!.n,1);
});
test('retry, crash/reopen, conflicting revision, and atomic publication preserve receipts',async t=>{
  ...
  reopened.tick();assert.equal(otherMaterials.list().items.length,1);await reopened.close();other.close();
  const before=materials.list().items[0];const ids=materials.evidenceIds(before.ref);
  await sources.upsert('coding',item(2));runtime.tick();const after=materials.list().items[0];assert.notEqual(after.revision,before.revision);assert.equal(after.sequence,2);assert.equal(materials.isCurrentEvidence(ids[0]),false);
  assert.ok(materials.get(before.ref));assert.equal(store.db.prepare('SELECT count(*) n FROM material_fts').get()!.n,1);
});
test('Cordis uninstall blocks archive work, ordinary records stay supported, index can be disabled',async t=>{
  ...
  await sources.upsert('coding',item(1));runtime.configure('coding',{index:false,memory:false,settleSeconds:0});runtime.tick();assert.equal(materials.list({query:'Generated'}).items.length,0);
...
test('model extraction first reads the assembled conversation, never raw upload records',async t=>{
  ...
  await sources.upsertBatch('coding',[item(1),item(2,'session-a','Second generated message.')]);runtime.tick();
  let calls=0;const pipeline=new MemoryPipeline({store,memories:reader.memories,configured:()=>true,model:()=> 'fixture',query:async input=>{
...
test('short Chinese search, source revision moves, explicit erasure and stable duplicate heads',async t=>{
  ...
  await sources.upsert('coding',item(1,'session-a','中文需求'));runtime.tick();
  assert.equal(materials.list({query:'中文'}).items.length,1);
  await sources.upsert('coding',{...item(1,'session-b','Moved requirement'),revision:'2',observedAt:'2026-09-24T02:00:00Z'});runtime.tick();
  assert.equal(materials.list({query:'中文'}).items.length,0);assert.equal(materials.list({query:'Moved'}).items.length,1);
  await sources.upsert('coding',{...item(1,'session-a','中文需求'),observedAt:'2026-09-24T03:00:00Z'});runtime.tick();
...
test('HTTP source upload, material search and actual agent bridge cite assembled evidence end to end',async t=>{
  ...
  const upload=await node.app.inject({method:'POST',url:'/api/sources/coding/items/batch',headers,payload:{items:[item(1,'session','Fixture bridge proof'),item(2,'session','Second message')]}});assert.equal(upload.statusCode,200);assert.equal(upload.json().receipts.length,2);
...
test('fragment boundaries preserve exact text and partial messages wait for their remaining parts',async t=>{
  ...
  await sources.upsert('coding',first);runtime.tick();const partial=materials.list().items[0];assert.equal(partial.coverage.state,'partial');assert.equal(store.db.prepare('SELECT count(*) n FROM material_memory_work').get()!.n,0);
  const second={...first,externalId:'part-two',text:'The end.',document:{...first.document,coding:{...first.document.coding,part:1}}};
  await sources.upsert('coding',second);runtime.tick();const full=materials.list().items[0];assert.equal(full.coverage.state,'complete');
...
test('explicit pipeline replacement rebuilds groups without introducing raw records',async t=>{
  const {runtime,sources,materials}=await fixture(t);await sources.upsert('coding',item(1));runtime.tick();
  const installed=runtime.registry.get('mote.coding')!;
  runtime.registry.register({...installed,id:'fixture.replacement',version:'3',priority:1,organize:input=>({...installed.organize!(input)!,title:'Replacement title'})});
  runtime.configure('coding',{pipelineId:'fixture.replacement',memory:false});runtime.tick();assert.equal(materials.list().items[0].title,'Replacement title');assert.equal(runtime.options('coding').pipelineId,'fixture.replacement');
});
...
test('file journal preserves both groups when a move is interrupted before SQL commit',async t=>{
  ...
  await sources.upsert('coding',item(1,'old','Previous body'));runtime.tick();
  ...
  await sources.upsert('coding',moved);runtime.tick();
...
test('encrypted raw archive survives bulk decrypt and a keyless restart',async t=>{
  ...
  await runtime.close();store.close();store=new Store(directory);const materials=new MaterialStore(store);runtime=new SourcePipelineRuntime(store,materials,[codingSourcePlugin]);await runtime.ready;runtime.tick();
