...
test('dependent artifact lineage includes every ancestor, including a different original',async t=>{
 ...
 runtime.registry.register({id:'fixture.chain',version:'1',lane:'aggregate',async process(input){if(input.artifacts.length)assert.equal(input.artifacts[0].outputs[0].text,'Derived');return [{kind:'derived',text:'Derived',metadata:{}}];}});
 const jobs=runtime.enqueue([{name:'parent',processor:'fixture.chain',inputs:[a.id]},{name:'child',processor:'fixture.chain',inputs:[b.id],dependsOn:['parent']}]);await runtime.tick();await runtime.tick();const child=JSON.parse(String(store.db.prepare('SELECT json FROM processing_jobs WHERE id=?').get(jobs.child)!.json));assert.equal(store.archive.get(child.outputs[0])!.members.length,2);store.delete(a.id);assert.equal(store.archive.get(child.outputs[0]),undefined);
});
