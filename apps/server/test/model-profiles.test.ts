...
test('profiles route each request and feature independently; credentials, restarts and deletes are scoped',async t=>{
  ...
  await node.sources.upsert('fixture',{externalId:'fixture',revision:'1',observedAt:'2026-01-01T00:00:00Z',text:'Generated memory evidence',kind:'file',layer:'original'});
...
test('module model overrides reach chat, insights, immediate memory and persisted memory batches',async t=>{
  ...
  await node.sources.upsert('module-fixture',{externalId:'fixture',revision:'1',observedAt:'2026-01-01T00:00:00Z',text:'Generated memory fixture',kind:'file',layer:'original'});
  await node.materialOrganizer.tick();
  const fixtureMaterial=node.materials.get(materialId('module-fixture','fixture'));
  assert.ok(fixtureMaterial);
  const created=await node.app.inject({method:'POST',url:'/api/memory-jobs',headers,payload:{evidenceIds:node.materials.evidenceIds(fixtureMaterial.ref)}});
