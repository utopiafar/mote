...
test('review coverage unions two devices, persists gaps, and versions late data without altering the old report',async t=>{
 ...
 const generate=async(id:string)=>runs.perform(id,scope,async(_observe,_signal,snapshot)=>({runId:randomUUID(),answer:'Generated report version '+snapshot.version,citations:[],trace:[],snapshot}));
...
test('late originals change the next receipt without failing an in-flight review',async t=>{
 ...
 await store.ingest(capture('other'));await store.ingest(capture('selected','2025-02-02T00:00:30.000Z'));
 assert.equal(createInsightSnapshot(store,randomUUID(),selected).scopeFingerprint,snapshot.scopeFingerprint);
 let release!:()=>void,captured!:InsightSnapshot;const id=randomUUID(),pending=runs.perform(id,selected,async(_observe,_signal,receipt)=>{captured=receipt;await new Promise<void>(resolve=>release=resolve);return {runId:randomUUID(),answer:'Generated report despite a late arrival',citations:[],trace:[]};});
 await new Promise(resolve=>setImmediate(resolve));await store.ingest({...capture('selected'),id:randomUUID()});
 assert.notEqual(createInsightSnapshot(store,randomUUID(),selected).scopeFingerprint,captured.scopeFingerprint);
 release();const result=await pending;
 assert.equal(runs.get(id).status,'completed');assert.equal(runs.get(id).resultRunId,result.runId);
 assert.equal(runs.detail(id).result.snapshot.scopeFingerprint,captured.scopeFingerprint);
 assert.equal(store.db.prepare('SELECT count(*) n FROM insights').get()!.n,1);
});
