...
test('PDF, DOCX and XLSX share text and locators across Desktop, automatic import, and central processing',async t=>{
 ...
 for(const value of fixtures){
  ...
  const job=await imports.create({files:[{name:value.name,mimeType:value.mime,dataBase64:value.bytes.toString('base64')}],processing:'automatic'}),result=await imports.prepare(job.id);assert.equal(result.status,'completed');assert.equal(result.captureIds.map(id=>store.evidence([id])[0].ocrText).join(''),desktop.text);assert.deepEqual(archive.read(result.files[0].id),value.bytes);
