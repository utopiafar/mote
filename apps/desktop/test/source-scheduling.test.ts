...
it('keeps scanning/staging files locally with no connection, restores them, and copies original versions only for confirmed first binding', async () => {
  ...
  const received: any[] = [];
  vi.mocked(fetch).mockImplementation(async (_url, init) => { if(init!.method==='GET')return new Response(JSON.stringify({revision:null}));const manifest = JSON.parse(init!.body as string);const body=manifest.item??manifest; received.push(body); return new Response(JSON.stringify(init!.method === 'PUT' ? { id: 'b67c1b84-f2cd-4e59-bf67-215545a882dc', sourceId: app.status()[0].source.id, externalId: body.externalId, revision: body.revision, duplicate: false } : { ...body, id: app.status()[0].source.id }), { status: 200 }); });
