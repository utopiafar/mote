...
describe('acknowledgment-gated uploads', () => {
  ...
  it('splits oversized capture batches and preserves the exact receipt identities',async()=>{
    ...
    vi.stubGlobal('fetch',vi.fn(async(url:string,options:RequestInit)=>{
      ...
      return body.captures.length>2?new Response('too large',{status:413}):new Response(JSON.stringify({results:body.captures.map((c:{id:string})=>({id:c.id,status:201}))}),{status:200});
    }));
  ...
  it.each([404,405])('retains the legacy single-record protocol only for missing batch routes (%s)',async status=>{
    const urls:string[]=[];vi.stubGlobal('fetch',vi.fn(async(url:string,options:RequestInit)=>{urls.push(url);return url.endsWith('/batch')?new Response('',{status}):new Response(JSON.stringify({id:JSON.parse(await new Response(options.body).text()).id}),{status:201});}));
    const entries=[{event:event()},{event:{...event(),id:'00000000-0000-4000-8000-000000000002'}}];
    const receipts=await uploadCaptureBatch({...defaultConfig(),token:'synthetic-token'},entries);expect(receipts.size).toBe(2);expect(urls).toHaveLength(3);
  });
