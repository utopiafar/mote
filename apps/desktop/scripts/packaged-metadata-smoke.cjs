...
(async () => {
  ...
  try {
    ...
    await queue.enqueue({...activity,id:randomUUID(),capturedAt:'2026-09-14T00:01:15.000Z',durationMs:15000});
    const restarted = new DurableQueue(join(directory, 'queue'), migrated); await restarted.initialize(); assert.equal(restarted.stats().depth, 3);
    assert.deepEqual((await restarted.exportArchive()).records.find(record => record.event.id === activity.id).event, {...activity,stateSeries:{version:1,samples:[{at:activity.capturedAt,durationMs:0},{at:'2026-09-14T00:01:15.000Z',durationMs:15000}]}});
