...
(async () => {
  try {
    ...
    process.stdout.write(JSON.stringify({ok:true, fixtureOnly:true, packagedAsarModules:true, packagedNativeHelper:true, decision, durationMs:gate.status().lastDurationMs}) + '\n');
  } finally { gate.close(); }
