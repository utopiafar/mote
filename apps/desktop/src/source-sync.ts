...
export class SourceSync {
  ...
  async stage(scan: SourceScan, trackDeletions: boolean, observedAt = new Date().toISOString(), initialSync: 'all'|'new_only'='all'): Promise<number> {
    ...
    if(initialSync==='new_only'&&!next.initialized&&Object.keys(next.known).length===0){
      ...
      if(Buffer.byteLength(JSON.stringify(next))>this.limits.maxBytes)throw new Error('首次同步基线达到来源缓存上限');
      next.initialized=scan.complete;await this.commit(next);return 0;
    }
    ...
    for (const item of scan.items) if(!baseline.has(item.externalId))stage(item);
