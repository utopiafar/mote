...
const messages: Record<string, string> = {
  release_not_found: '此渠道暂未提供可验证的发布清单。请稍后检查；旧版未签名发布不能用于应用内安装。',
  release_rate_limited: 'GitHub 暂时限制请求，请稍后重试。',
  invalid_manifest_signature: '发布清单签名无效，已停止更新。', unknown_release_key: '发布密钥不受此 App 信任，已停止更新。',
  asset_checksum_mismatch: '安装包校验失败，已删除不完整下载。请重新下载。', asset_size_mismatch: '安装包大小不符，已停止更新。',
  UPDATE_ARCHIVE_INVALID: '安装包目录、压缩格式或链接不安全，已停止解压。', UPDATE_BUNDLE_INVALID: '应用签名、版本或架构校验失败，未替换现有 App。',
  UPDATE_STAGE_CHANGED: '待安装应用在校验后发生变化，已停止安装，请重新下载。',
  UPDATE_OTHER_PROFILES_RUNNING: '同一个 App 还有其他 profile 正在运行。请先退出这些实例，再安装。',
  UPDATE_INSTALL_LOCKED: '另一个更新正在进行，或上次更新事务尚未结束。请保留应用旁的旧包并检查更新状态。',
  UPDATE_TARGET_NOT_WRITABLE: '应用目录不可写。请显示已验证安装包，用 Finder 手动替换 App。',
  UPDATE_HELPER_NOT_READY: '安装助手未能就绪，当前 App 保持运行。请检查应用目录并重试。',
};
export function updateMessage(error: unknown): string {
  const key = (error as { code?: string })?.code || (error as Error)?.message;
  return messages[key] || '更新未完成，现有 App 与资料保持不变。请检查网络、磁盘空间后重试。';
}
export class DesktopUpdater {
  ...
  async initialize(): Promise<void> {
    ...
    catch (error) { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') this.value.message = '更新渠道配置无法读取，暂按稳定渠道检查。现有资料未变动。'; }
    await this.cleanupDownloads().catch(() => { this.value.message = '旧更新缓存暂时无法清理，请检查磁盘权限；采集资料未变动。'; });
  ...
  async startupCompleted(): Promise<void> {
    ...
    if (recovery === 'blocked') { this.value.state = 'error'; this.value.message = '上次更新未能安全确认。已保留应用旁的恢复包，未删除资料。请备份并通过 Finder 手动恢复或替换 App。'; return; }
    try {
      ...
      for (const entry of recent) {
        try {
          ...
          if (result.state === 'installed' && result.version === this.options.currentVersion) this.value.message = '更新已安装并完成启动确认。原有 profile、草稿、队列和模型保持原位。';
          else if (result.state === 'rolled_back') { this.value.state = 'error'; this.value.message = '新版启动未确认，已自动恢复旧版 App。原有资料保持原位，可重新检查更新。'; }
          else if (result.state === 'failed') { this.value.state = 'error'; this.value.message = result.backupRetained ? '更新未完成，旧应用恢复包已保留。请先备份并通过 Finder 检查应用旁的恢复包。' : '上次更新未完成，现有 App 与资料保持原位。可重新检查更新。'; }
          break;
  ...
  async setChannel(channel: unknown): Promise<UpdateStatus> {
    if (channel !== 'stable' && channel !== 'preview') throw new Error('更新渠道无效');
    await this.cancel();
    ...
    Object.assign(this.value, { state: 'idle', message: '渠道已保存。点击检查更新。', availableVersion: undefined, notesUrl: undefined, received: 0, total: 0 }); return this.status();
  }
  async check(): Promise<UpdateStatus> {
    if (this.task || this.value.state === 'installing') return this.status();
    const controller = new AbortController(); this.controller = controller;
    this.value.state = 'checking'; this.value.message = '正在检查发布清单和签名…';
    this.task = (async () => {
      try {
        ...
        Object.assign(this.value, { state: result.available ? this.asset ? 'available' : 'error' : 'up_to_date', availableVersion: result.manifest.version, notesUrl: result.manifest.notesUrl, checkedAt: new Date().toISOString(), received: 0, total: this.asset?.size || 0,
          message: result.available ? this.asset ? '发现已签名的新版本，可以下载并校验。' : '此版本尚无适合当前 Mac 架构的更新包。' : '当前版本已经是此渠道最新版本。' });
      } catch (error) { this.value.state = 'error'; this.value.message = controller.signal.aborted ? '更新检查已取消。' : updateMessage(error); }
    })().finally(() => { this.task = undefined; this.controller = undefined; });
    await this.task; return this.status();
  }
  download(): UpdateStatus {
    if (this.task || this.value.state === 'installing') return this.status();
    if (!this.manifest || !this.asset || compareVersions(this.manifest.version, this.options.currentVersion) <= 0) throw new Error('请先检查可用更新');
    const asset = structuredClone(this.asset), manifest = this.manifest, controller = new AbortController(); this.controller = controller;
    this.staged = undefined; this.archive = undefined;
    Object.assign(this.value, { state: 'downloading', message: '下载经过签名清单绑定的安装包…', received: 0, total: asset.size });
    this.task = (async () => {
      ...
      try {
        ...
        controller.signal.throwIfAborted(); this.value.state = 'verifying'; this.value.message = '校验 ZIP 路径、应用签名、版本、架构和完整文件树…';
        ...
        this.value.state = 'ready'; this.value.message = '安装包与应用已校验。安装前将保存当前操作并退出 App。';
      } catch (error) {
        await rm(folder, { recursive: true, force: true }).catch(() => {});
        this.value.state = 'error'; this.value.message = controller.signal.aborted ? '下载已取消。下次下载会从头开始。' : updateMessage(error);
      }
    })().finally(() => { this.task = undefined; this.controller = undefined; });
    return this.status();
  }
  ...
  async cancel(): Promise<UpdateStatus> {
    if (this.value.state === 'installing') throw new Error('安装交接已开始，请等待新版本启动');
    this.controller?.abort(); await this.task; return this.status();
  }
  async install(beforeQuit: () => Promise<void>, quit: () => void): Promise<void> {
    if (this.task || this.value.state !== 'ready' || !this.staged || !this.manifest || !this.asset || !this.options.bundlePath) throw new Error('没有可安装的已验证更新');
    this.value.state = 'installing'; this.value.message = '准备应用替换，等待保存当前操作…';
