...
export function normalizeAppCollectionRules(value: unknown): Record<string, CollectionMode> {
  if (!value || typeof value !== 'object' || Array.isArray(value) || Object.keys(value).length > 500) throw new Error(moteText("应用采集规则必须为有效列表，最多 500 项"));
  const result: Record<string, CollectionMode> = {};
  for (const [id, mode] of Object.entries(value)) {
    if (!id.trim() || id !== id.trim() || id.length > 256 || /[\x00-\x20\x7f]/.test(id) || ['__proto__', 'constructor', 'prototype'].includes(id)) throw new Error(moteText("应用 Bundle ID 无效"));
