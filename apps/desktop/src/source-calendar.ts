...
export async function calendarHelper(path: string, command: 'calendar-permission' | 'calendar-list' | 'calendar-scan', input?: unknown, signal?: AbortSignal): Promise<unknown> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const abort = () => { child.kill('SIGKILL'); finish(new Error('日历读取已取消')); };
    const timer = setTimeout(() => { child.kill('SIGKILL'); finish(new Error('日历读取超时，可重试或检查系统权限')); }, command === 'calendar-permission' ? 120000 : 20000);
    signal?.addEventListener('abort', abort, { once: true }); if (signal?.aborted) abort();
    child.stdout.on('data', (chunk: Buffer) => { bytes += chunk.length; if (bytes > 9 * 1024 * 1024) { child.kill('SIGKILL'); finish(new Error('日历读取超过安全上限')); } else chunks.push(chunk); });
    child.on('error', () => finish(new Error('无法启动日历助手，请检查应用安装')));
    child.on('close', code => { if (done) return; if (code !== 0) return finish(new Error('日历读取失败，请检查日历权限后重试')); try { finish(undefined, JSON.parse(Buffer.concat(chunks).toString('utf8'))); } catch { finish(new Error('日历助手返回格式无效')); } });
...
export function decodeCalendarChoices(raw: unknown): CalendarChoice[] {
  ...
  if (!Array.isArray(value.calendars) || value.calendars.length > 500 || value.calendars.some(c => typeof c.id !== 'string' || c.id.length > 1000 || typeof c.title !== 'string' || c.title.length > 2000)) throw new Error('日历列表格式无效');
...
export function decodeCalendarScan(raw: unknown, options: SourceOptions, scope: { start: string; end: string }): SourceScan {
  ...
  if (data.missingCalendar) throw new Error('所选日历已不可用；已暂停删除判断，请重新选择');
  if (!Array.isArray(data.events) || data.events.length > 2000 || typeof data.complete !== 'boolean') throw new Error('日历数据格式无效');
