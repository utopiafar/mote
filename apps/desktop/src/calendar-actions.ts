...
export class CalendarActions {
 ...
 connect(){return this.serial(async()=>{const value=await this.io.native('calendar-permission');if(value.permission!=='granted')throw Error('请允许日历访问后重试');await this.io.request('/api/actions/targets',{deviceId:this.config.deviceId,deviceName:this.config.deviceName,calendars:value.calendars.filter((c:any)=>c.writable===true).map((c:any)=>({id:c.id,title:c.title}))});await this.io.write('enabled','1');return {deviceId:this.config.deviceId};});}
 ...
 execute(id:string){return this.serial(async()=>{
  if(!/^[0-9a-f-]{36}$/i.test(id))throw Error('日程编号无效');
  const action=await this.io.request(`/api/actions/${id}/claim`,{deviceId:this.config.deviceId}) as ActionProposal;
  if(action.id!==id||action.kind!=='calendar.create'||action.target?.deviceId!==this.config.deviceId||!action.operationId)throw Error('日程确认信息无效');
  moteActionMarker(action.operationId);
  if(action.status==='succeeded')return;
  if(!['executing','uncertain'].includes(action.status))throw Error('日程尚未获准执行');
  ...
  try{
   if(!externalId){await this.io.write(action.operationId,'attempting');const result=await this.io.native('calendar-create',{id:action.id,calendarId:action.target.calendarId,...action.event,description:calendarDescription(action),createAllowed:!previous});if(typeof result.externalId!=='string'||!result.externalId)throw Error('保存结果待核实，请查看系统日历后重试');externalId=result.externalId;await this.io.write(action.operationId,externalId!);}
  ...
  }catch{await this.io.request(`/api/actions/${id}/receipt`,{deviceId:this.config.deviceId,operationId:action.operationId,status:'uncertain'}).catch(()=>{});throw Error('保存结果待核实。请检查日历权限与网络后再次核实；不会重复插入。');}
 });}
}
export function nativeCalendarActions(config:Config,helper:string,directory:string){
 ...
 return new CalendarActions(config,{
  async request(path,body){const r=await fetch(origin+path,{method:body?'POST':'GET',redirect:'error',signal:AbortSignal.timeout(30000),headers:{Authorization:`Bearer ${config.token}`,'Content-Type':'application/json'},body:body?JSON.stringify(body):undefined});if(!r.ok)throw Error('中央日程操作失败，请刷新或重新授权');return r.json();},
