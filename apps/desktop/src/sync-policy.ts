...
export function decideSync(config: SyncSettings, pending: PendingSync, now = Date.now(), explicit = false): SyncStatus & { ready: boolean } {
  ...
  if (!config.serverUrl || !config.token) return { ...base, state: 'unconfigured', message: '仅保存在本机；连接中央节点后可同步', ready: false };
  if (explicit) return { ...base, state: 'idle', message: '准备同步本地记录', ready: true };
  if (base.mode === 'manual') return { ...base, state: 'manual', message: pending.pendingRecords ? '记录已保存在本机，等待手动同步' : '手动同步已启用', ready: false };
  if (!pending.pendingRecords && !pending.pendingUpdates) return { ...base, state: 'idle', message: '已同步，等待新记录', ready: false };
  ...
  if (due <= now) return { ...base, state: 'idle', message: '本地记录已就绪，准备同步', ready: true };
  return { ...base, state: 'waiting', message: base.mode === 'batch' ? `等待累计 ${config.syncBatchSize} 条，或到达最长等待时间` : '记录已保存在本机，等待下次同步', nextUploadAt: new Date(due).toISOString(), ready: false };
}
