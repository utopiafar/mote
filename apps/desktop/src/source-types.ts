...
export function normalizeSourceOptions(input: unknown): SourceOptions {
  ...
  if (!['snapshot', 'reference'].includes(v.retention) || !Number.isInteger(v.intervalSeconds) || v.intervalSeconds < 30 || v.intervalSeconds > 3600 || typeof v.trackDeletions !== 'boolean') throw new Error(moteText("来源同步间隔为 30–3600 秒，保留方式为快照或引用"));
