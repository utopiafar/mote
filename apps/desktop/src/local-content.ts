...
export function configureLocalContent(value: ContentPolicy): void {
  if (value.key && value.key.length !== 32 || value.enabled && !value.key) throw new Error('本地内容加密密钥不可用');
...
export function encodeLocalContent(value: string | Uint8Array, selected = policy): Buffer {
  ...
  if (!selected.key || selected.key.length !== 32) throw new Error('本地内容加密密钥不可用');
...
export function decodeLocalContent(bytes: Buffer, selected = policy): Buffer {
  ...
  if (!selected.key || selected.key.length !== 32) throw new Error('无法读取本地加密内容：原内容密钥不可用');
  const offset = encryptedPrefix.length;
  if (bytes.length < offset + 28) throw new Error('本地加密文件不完整，原文件已保留');
...
export class LocalContentKeyStore {
  ...
  async initialize(enabled: boolean): Promise<void> {
    try {
      ...
      if (saved.version !== 1 || typeof saved.encryptedKey !== 'string' || !this.secrets.available()) throw new Error('本地内容密钥存储不可用');
      const value = this.secrets.decrypt(Buffer.from(saved.encryptedKey, 'base64'));
      if (!/^[a-f0-9]{64}$/.test(value)) throw new Error('本地内容密钥无效');
  ...
  async setEnabled(enabled: boolean): Promise<void> {
    if (enabled && !this.key) {
      if (!this.secrets.available()) throw new Error('系统密钥存储不可用，无法启用本地内容加密');
...
export function emptyDecryptionProgress(): DecryptionProgress { return { state: 'idle', total: 0, processed: 0, decrypted: 0, skipped: 0, failed: 0, message: '尚未运行批量解密' }; }
...
export async function decryptLocalContent(roots: string[], signal: AbortSignal, progress: (value: DecryptionProgress) => void, selected = policy): Promise<DecryptionProgress> {
  const result: DecryptionProgress = { ...emptyDecryptionProgress(), state: 'running', message: '正在读取本机文件目录…' };
  progress({ ...result });
  const files: string[] = [];
  ...
  for (const path of files) {
    ...
    result.processed++; result.message = '正在逐个解密本机内容'; progress({ ...result });
    await yieldTurn();
  }
  result.state = signal.aborted ? 'cancelled' : result.failed ? 'failed' : 'completed';
  result.message = `${signal.aborted ? '已取消，未处理文件保持原样' : '处理结束'}：解密 ${result.decrypted}，跳过 ${result.skipped}，失败 ${result.failed}；失败文件保留`;
