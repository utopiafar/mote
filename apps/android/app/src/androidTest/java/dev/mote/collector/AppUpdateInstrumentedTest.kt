...
class AppUpdateInstrumentedTest {
    ...
    @Test fun nativeUpdatePageDoesNotGrantInstallPermissionOrTriggerNetwork() {
        ...
        ActivityScenario.launch(AppUpdatesActivity::class.java).awaitUiText(if (BuildConfig.APPLICATION_ID.endsWith(".dev")) "下载 DEV 安装包" else "检查更新").use { scenario -> scenario.onActivity { activity ->
            ...
            walk(activity.window.decorView); assertTrue(labels.contains(if (BuildConfig.APPLICATION_ID.endsWith(".dev")) "下载 DEV 安装包" else "检查更新")); assertFalse(labels.contains("交给系统安装"))
        } }
