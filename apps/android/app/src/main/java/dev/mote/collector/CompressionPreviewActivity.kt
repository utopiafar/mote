...
class CompressionPreviewActivity : Activity() {
    ...
    override fun onCreate(state: Bundle?) {
        ...
        text("图片压缩预览", 27f)
        text("用生成的图文样张比较小字、渐变和细线。只在内存处理，不读取或上传屏幕。")
        val label = text("JPEG 质量 $quality · 不是文件压缩百分比")
        body.addView(SeekBar(this).apply {
            min = 40; max = 95; progress = quality
            setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                override fun onProgressChanged(bar: SeekBar?, value: Int, user: Boolean) { quality = value; label.text = "JPEG 质量 $value · 不是文件压缩百分比"; if (user) { revision++; apply.isEnabled = false } }
        ...
        stats = text("正在生成预览…")
        text("原始示例 · PNG（点击放大）", 17f)
        originalView = ImageView(this).apply { contentDescription = "原始示例"; scaleType = ImageView.ScaleType.FIT_CENTER; body.addView(this, LinearLayout.LayoutParams(-1, moteDp(260))); setOnClickListener { enlarge(0) } }
        text("压缩结果 · JPEG（点击放大）", 17f)
        compressedView = ImageView(this).apply { contentDescription = "压缩结果"; scaleType = ImageView.ScaleType.FIT_CENTER; body.addView(this, LinearLayout.LayoutParams(-1, moteDp(260))); setOnClickListener { enlarge(1) } }
        text("放大后可双指缩放、拖动检查小字；双击复位。文件大小与缩放比例依据本次编码实测。")
        apply = Button(this).apply { text = "将参数带回采集设置"; isEnabled = false; setOnClickListener { setResult(RESULT_OK, Intent().putExtra("quality", quality).putExtra("maxSide", maxSide)); finish() } }; body.addView(apply)
        text("返回采集设置后点击保存，才会应用到后续采集。")
    ...
    private fun fixture(): Bitmap {
        ...
        canvas.drawText("Mote · 把细节留在记录里", 70f, 150f, paint)
        repeat(34) { i -> paint.textSize = (18 + i % 4 * 5).toFloat(); canvas.drawText("${i + 1}  图片清晰度 / Small text Aa 0123456789 → 阅读与记录", 75f, 270f + i * 42, paint) }
        ...
        paint.color = Color.DKGRAY; paint.textSize = 27f; canvas.drawText("生成示例 · 不读取屏幕 · 不上传", 75f, 2400f, paint)
        return image
    }
    ...
    private fun render() {
        ...
        apply.isEnabled = false; stats.text = "正在生成压缩预览…"; dialog?.dismiss()
        executor.execute {
            ...
            try {
                ...
                val result = BitmapFactory.decodeByteArray(jpeg, 0, jpeg.size) ?: error("无法解码 JPEG"); decoded = result
                val message = "1440 × 2560 → ${output.width} × ${output.height}\n边长缩放 ${"%.1f".format(scale * 100)}% · PNG ${png.size / 1024} KB → JPEG ${jpeg.size / 1024} KB\n文件大小为原图的 ${"%.1f".format(jpeg.size * 100.0 / png.size)}%"
                ...
                runOnUiThread { if (!isDestroyed && stamp == revision) stats.text = "预览未完成，请调整参数重试。" }
            }
        }
    }
    private fun enlarge(index: Int) {
        if (images.size < 2) return
        val image = BulkDedupeImageView(this).apply { setImageBitmap(images[index]); contentDescription = "双指缩放、拖动，双击复位" }
        dialog = AlertDialog.Builder(this).setTitle(if (index == 0) "原始示例 · 双指放大" else "压缩结果 · 双指放大")
            .setView(image).setPositiveButton("关闭", null).setNeutralButton(if (index == 0) "看压缩结果" else "看原图") { _, _ -> enlarge(1 - index) }.create()
