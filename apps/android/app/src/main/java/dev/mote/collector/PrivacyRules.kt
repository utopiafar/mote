...
data class Mask(val left: Float, val top: Float, val right: Float, val bottom: Float) {
    init {
        require(listOf(left, top, right, bottom).all { it.isFinite() && it in 0f..1f }) { "遮罩坐标必须在 0..1" }
        require(left < right && top < bottom) { "遮罩右下坐标必须大于左上坐标" }
    }
    companion object {
        fun parse(value: String): List<Mask> = value.lineSequence().map { it.trim() }.filter { it.isNotEmpty() }.map {
            ...
            require(v.size == 4) { "每行遮罩需要 left,top,right,bottom 四个坐标" }
...
object PrivacyRules {
    ...
    fun excludedReason(excluded: Set<String>, visiblePackages: Set<String>, trustworthy: Boolean): String? {
        ...
        if (!trustworthy || visiblePackages.isEmpty() || visiblePackages.any { it.isBlank() }) return "无法可靠识别窗口，已暂停：排除规则要求完整应用信息"
        if (visiblePackages.any { it in excluded }) return "当前屏幕包含你排除的应用"
        return null
    }

    fun validateEndpoint(value: String, debugHttp: Boolean, isDebug: Boolean): String {
        val uri = URI(value.trim())
        require(!uri.host.isNullOrBlank() && uri.rawUserInfo == null && uri.rawQuery == null && uri.rawFragment == null) { "请输入不含账号、参数或片段的节点 URL" }
        require(uri.path.isNullOrEmpty() || uri.path == "/") { "节点 URL 不应含路径" }
        ...
        require(uri.scheme == "https" || (uri.scheme == "http" && privateHost && debugHttp && isDebug)) { "请使用 HTTPS；仅 debug APK 可显式允许私有 IP 的 HTTP" }
