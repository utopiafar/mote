...
object AppReleaseVerifier {
    ...
    fun verify(raw: ByteArray, publicKeyPem: String, expected: UpdateConfig, expectedVersion: String? = null): AppRelease {
        ...
        try {
            ...
            // The platform JSON parser recurses without a depth limit; cap nesting before parsing unsigned input.
            val envelopeText = utf8(raw); StrictJson.validate(envelopeText)
            val envelope = JSONObject(envelopeText); exactKeys(envelope, setOf("schemaVersion", "keyId", "payload", "signature"))
