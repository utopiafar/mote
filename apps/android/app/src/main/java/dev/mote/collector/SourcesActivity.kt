        ...
        task.start("正在读取所选文件…", { operationStatus.text = it }, {
            ...
            try {
                ...
                localSources().sources().find { it.uri == uri.toString() }
                    ?: LocalSource(name = (name ?: if (requestCode != 402) "选择的文件目录" else "选择的文件").take(200), kind = "local-files", uri = uri.toString(), tree = requestCode != 402, retention = if (requestCode == 405) "archive" else "snapshot", extensions = if (requestCode == 405) "m4a,mp3,wav,aac,amr,ogg,flac,opus" else "md,txt,json,csv,ics")
