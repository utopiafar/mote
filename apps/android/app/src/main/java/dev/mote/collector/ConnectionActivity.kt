...
class ConnectionActivity : Activity() {
    ...
    private val refresh = object : Runnable {
        override fun run() {
            ...
            if (refreshRequested && ::currentNode.isInitialized && !working && !task.busy && !refreshTask.busy) {
                ...
                refreshTask.start("正在刷新已保存连接…", { currentNode.text = it }, {
                    ...
                    result.onSuccess { (config, id) -> currentConfig = config; deviceId = id; refreshCurrentNode() }
                        .onFailure { currentNode.text = "读取当前连接失败，请重新打开此页重试" }
                }
            }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        task.start("正在读取已保存连接…", { label.text = it }, {
            ...
            result.onSuccess { initial -> currentConfig = initial.config; deviceId = initial.deviceId; buildUi(initial) }
                .onFailure { label.text = "无法读取已保存连接，请检查本机存储后重试" }
        }
    }
    private fun buildUi(initial: InitialConnection) {
        ...
        button("复制本设备 ID") { getSystemService(android.content.ClipboardManager::class.java).setPrimaryClip(android.content.ClipData.newPlainText("Mote device ID", deviceId)); Toast.makeText(this, "已复制设备 ID，不含凭据", Toast.LENGTH_SHORT).show() }
        ...
        button("确认连接此节点") {
            ...
            AlertDialog.Builder(this).setTitle("确认连接节点").setMessage("${invite.serverUrl}\n\n将使用已有设备 ID 和当前采集设置。只获取这台设备的采集凭据；设置、队列和模型不会清空。同节点重新配对将使用新凭据继续同步本机待上传截图、笔记和来源。首次连接将把尚未绑定的本机截图、笔记与来源记录绑定到上方节点，并按已选择的同步方式发送。请确认这是你自己的档案地址。已经绑定其他节点的待同步记录不能改投此处。")
                .setNegativeButton("取消", null).setPositiveButton("连接") { _, _ ->
                    ...
                    changeConnection(invite.serverUrl, "正在兑换邀请并校验设备身份…", connected = {
                        input.text.clear(); parsed = null; preview.text = "连接成功：${invite.serverUrl}"
                    }) {
        ...
        button("恢复中断的连接确认") {
            working = true
            task.start("正在读取待恢复连接…", { status.text = it }, {
                ConnectionClient(applicationContext).pendingServer() ?: throw ConnectionFailure("no_pending")
            }) { result ->
                working = false
                result.onSuccess { server ->
                    AlertDialog.Builder(this).setTitle("恢复已兑换连接").setMessage("$server\n\n将重新联网校验已兑换并加密保存的凭据。确认后，尚未绑定的本机记录会绑定到上方节点并按同步设置发送。")
                        .setNegativeButton("取消", null).setPositiveButton("恢复") { _, _ ->
                            val chosenName = name.text.toString().trim(); val debug = allowHttp.isChecked
                            changeConnection(server, "正在重新校验设备连接…") { ConnectionClient(applicationContext).resume(chosenName, debug, bindLocal = true); "中断的连接已恢复。" }
                        }.show()
                }.onFailure(::showFailure)
            }
        }
        button("测试已保存的连接") {
            run("正在测试节点和设备凭据…", work = { ConnectionClient(applicationContext).test() }) { scope ->
                "连接正常 · ${if (scope == "collector") "本设备采集权限" else "手工配置的管理员权限"}"
            }
        }
        status = text("${message(initial.status)}\n状态记录时间：${initial.statusAt.takeIf { it > 0 }?.let { Instant.ofEpochMilli(it) } ?: "未测试"}")
    ...
    private fun refreshCurrentNode() {
        currentNode.text = "当前节点：${currentConfig.server.ifBlank { "尚未配置" }}\n设备 ID：$deviceId\n已有设备请让中央生成绑定此设备 ID 的邀请；设备身份不会重置。"
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 1) {
            ...
            run("正在读取所选连接文件…", work = {
                ...
                input.setText(raw); parsed = invite; preview.text = "将连接：${invite.serverUrl}\n有效至：${invite.expiresAt}\n请确认后连接。"
                "连接文件已解析，尚未联网。"
            }
        }
    }
