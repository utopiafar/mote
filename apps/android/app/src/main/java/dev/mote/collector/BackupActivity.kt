...
class BackupActivity : Activity() {
    ...
    override fun onCreate(state: Bundle?) {
        ...
        text("导入与导出", 28f)
        text("客户端配置", 20f)
        text("迁移采集、应用规则、同步、模型和诊断设置。系统权限和来源文件授权需在新设备重新授予。")
        secrets = CheckBox(this).apply { text = "导出时包含节点令牌（文件为明文，请妥善保管）"; isChecked = includeToken }.also(body::addView)
        button("导出配置 JSON") { includeToken = secrets.isChecked; create(10, "application/json", "mote-settings.json") }
        button("导入配置 JSON") { open(11, "application/json") }
        text("本机记录", 20f)
        text("备份本机仍保留的截图、应用活动、通知、媒体和随手记。ZIP 为明文，不含中央归档、来源文件原件、未提交草稿或模型文件。")
        button("导出本机记录 ZIP") { create(12, "application/zip", "mote-records.zip") }
        button("导入本机记录 ZIP") { open(13, "application/zip") }
        status = text("导入前可预览；同 ID 记录不会重复添加。")
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            10 -> task.start("正在导出配置…", { status.text = it }, {
            ...
            }) { result -> status.text = if (result.isSuccess) "配置已导出" else "导出未完成，请重试" }
            11 -> task.start("正在读取配置…", { status.text = it }, {
                val current = Settings(this).read()
                val bytes = requireNotNull(contentResolver.openInputStream(uri)).use { input ->
                    val out = java.io.ByteArrayOutputStream(); val buffer = ByteArray(8192)
                    while (true) { val count = input.read(buffer); if (count < 0) break; require(out.size() + count <= ConfigurationArchive.MAX_BYTES) { "配置文件过大" }; out.write(buffer, 0, count) }
                    out.toByteArray()
                 }
                ...
                AlertDialog.Builder(this).setTitle("导入客户端配置？")
                    .setMessage("采集间隔 ${next.intervalSeconds} 秒 · 保留 ${next.uploadedRetentionDays} 天\n应用规则 ${next.collectionRules.apps.size} 项\n${if (current.server == next.server) "中央节点不变" else "中央节点将改变；未携带令牌时需重新连接"}\n\n将替换当前偏好设置。已有记录和设备身份保留，系统权限不会自动开启。")
                    .setNegativeButton("取消", null).setPositiveButton("导入") { _, _ ->
                        RuntimeSettings.apply(this, next, expected = current) { applied -> status.text = if (applied.isSuccess) "配置已导入" else applied.exceptionOrNull()?.message ?: "配置导入失败，原设置保留" }
                    }.show()
            }.onFailure { status.text = "配置无法导入：${it.message ?: "文件无效"}" } }
            12 -> task.start("正在导出本机记录…", { status.text = it }, {
            ...
            }) { result -> status.text = result.fold({ "已导出 $it 条记录" }, { "导出未完成，请删除不完整文件后重试" }) }
            13 -> task.start("正在校验备份…", { status.text = it }, {
                ...
                if (staged.origin != settings.dataOrigin()) { staged.close(); error("备份属于不同中央节点，请先连接原节点") }
                if (closed) { staged.close(); error("页面已关闭") }
                prepared = staged; staged
            }) { result -> result.onSuccess { staged ->
                AlertDialog.Builder(this).setTitle("导入 ${staged.count} 条本机记录？")
                    .setMessage("已有记录保留，同 ID 自动去重。同步将切换为手动，检查导入结果后可立即同步。")
                    .setNegativeButton("取消") { _, _ -> staged.close(); prepared = null }
                    ...
                    .setPositiveButton("导入") { _, _ ->
                        ...
                        RuntimeSettings.apply(this, next, expected = current, change = {
                            ...
                            status.text = if (applied.isSuccess) "记录已导入 · 手动同步" else "导入未完成：${applied.exceptionOrNull()?.message}。已导入记录保留，可重试同一备份。"
                        }
                    }.show()
            }.onFailure { status.text = "备份无法导入：${it.message ?: "文件无效"}" } }
        }
    }
