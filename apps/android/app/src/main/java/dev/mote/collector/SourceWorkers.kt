    ...
    override fun doWork(): Result {
        ...
        return try {
            for (source in store.sources().filter { it.enabled }) {
                ...
                if (!SourceAccess.available(applicationContext, source)) { store.status(source.id, "permission"); continue }
                ...
                catch (_: Exception) { store.status(source.id, "provider"); SupportEvents.record(applicationContext, EventStage.SOURCE, EventCode.STORAGE) }
            }
            ...
            for (source in store.sources().filter { it.enabled }) {
                ...
                try {
                    ...
                    if (!store.state(source.id).optBoolean("registered")) {
                        ...
                        if (code !in 200..299 || registration?.optString("id") != source.id) { store.status(source.id, "http"); retry = true; continue }
                        ...
                        if (patchCode !in 200..299 || updated?.optString("id") != source.id) { store.status(source.id, "http"); retry = true; continue }
                    ...
                    while (submitted < 20) {
                        ...
                        if (code == 409) { store.status(source.id, "paused"); break }
                        if (code !in 200..299 || !SourceRules.validAck(source.id, body, ack)) {
                            store.status(source.id, "ack"); SupportEvents.record(applicationContext, EventStage.SOURCE, EventJournal.httpFailure(code), httpStatus = code); retry = true; break
                        }
                ...
                } catch (_: Exception) { store.status(source.id, "offline"); SupportEvents.record(applicationContext, EventStage.SOURCE, EventCode.NETWORK); retry = true }
            }
