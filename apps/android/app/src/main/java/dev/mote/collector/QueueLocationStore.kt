...
class QueueLocationStore(private val control: File, private val legacy: File, private val cipher: ByteCipher,
    ...
    private fun atomic(file: File, bytes: ByteArray) {
        ...
        try { FileOutputStream(temp).use { it.write(bytes); it.fd.sync() }; check(temp.renameTo(file)) { MoteI18n.text("无法持久保存存储位置") }; syncDirectory(file.parentFile!!) }
        finally { temp.delete() }
    }
    ...
    private fun identified(location: QueueLocation): File {
        ...
        check(directory.isDirectory && !java.nio.file.Files.isSymbolicLink(directory.toPath())) { MoteI18n.text("所选存储位置不可用，请重新连接该存储介质") }
        check(File(directory, IDENTITY).readText(Charsets.UTF_8) == location.id) { MoteI18n.text("存储目录不匹配，已保留原选择和数据") }
        return directory
    }
    ...
    fun assertCurrent(expected: QueueLocation) { check(load() == expected) { MoteI18n.text("存储位置已更新，请重试此操作") } }
    fun migrate(baseId: String, base: File, progress: (String) -> Unit = {}): QueueLocation {
        progress(MoteI18n.text("正在检查原存储完整性")); recover(); val source = load()
        ...
        check(base.isDirectory || base.mkdirs()) { MoteI18n.text("目标存储不可用") }
        check(base.usableSpace >= size + 1024 * 1024) { MoteI18n.text("目标空间不足，原目录保持不变") }
        ...
        check(targetDirectory.mkdir()) { MoteI18n.text("无法创建目标存储目录") }
        ...
        checkpoint("journal")
        try {
            val files = sourceDirectory.listFiles() ?: error(MoteI18n.text("无法读取原存储目录"))
            for ((index, file) in files.withIndex()) {
                progress(MoteI18n.text("正在复制并校验文件 {0}/{1}", index + 1, files.size))
                if (file.name == IDENTITY) continue
                check(file.isFile && !java.nio.file.Files.isSymbolicLink(file.toPath())) { MoteI18n.text("原存储包含无法迁移的条目") }
                ...
                check(file.length() == destination.length() && hash(file).contentEquals(hash(destination))) { MoteI18n.text("复制校验失败，原数据已保留") }
            ...
            progress(MoteI18n.text("正在验证目标记录与图片"))
    ...
    fun recover() {
        ...
        when (current) {
            ...
            else -> error(MoteI18n.text("存储迁移状态不一致，请保留应用数据"))
        }
    }
    private fun removeOwned(location: QueueLocation) {
        ...
        identified(location)
        for (entry in directory.listFiles() ?: error(MoteI18n.text("无法清理旧存储副本"))) {
            check(entry.isFile && !java.nio.file.Files.isSymbolicLink(entry.toPath())) { MoteI18n.text("旧目录包含未知子目录，副本已保留") }
            if (entry.name != IDENTITY) check(entry.delete()) { MoteI18n.text("旧存储副本清理未完成") }
        }
