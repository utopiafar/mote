...
class SyncRecoveryWorker(context: Context, params: WorkerParameters) : Worker(context, params) {
    ...
    private fun run(): Result {
        ...
        try {
            if (inputData.getString("syncStamp") != SyncSchedule.stamp(config)) { report("连接或同步策略已变化，本次操作已停止；请重新发起"); return Result.failure() }
            ...
            if (inputData.getBoolean("replay", false)) {
                ...
                report("已安排补传 $records 条本机采集记录及 $versions 个保留来源版本；实际上传进度见下方同步状态。冲突和中央已删除记录保持隔离。")
            ...
            repeat(20) {
                ...
                if (ids.isEmpty()) {
                    report("检查完成：中央可见 $present 条，不可见 $unavailable 条。检查范围为手机仍保留的采集记录；不可见可能是缺失、删除或无权限，不会自动恢复。来源快照由全量补传时核验。")
                    return Result.success()
                }
                ...
                if (code == 401 || code == 403) { report("检查失败：连接授权失效，请重新连接同一节点后重试"); return Result.failure() }
                if (code == 404) { report("中央节点尚不支持两端检查，请升级中央端代码；未修改本机状态"); return Result.failure() }
                check(code == 200) { "检查响应未确认" }
                val items = response?.getJSONArray("items") ?: error("检查响应缺失")
                ...
                report("已检查 ${present + unavailable} 条 · 中央可见 $present · 不可见 $unavailable")
            }
            return Result.retry()
        } catch (_: Exception) {
            report("操作未完成，已完成的步骤保留；请检查网络或存储。本次手动操作可再次点击继续，未删除任何副本。")
    ...
    companion object {
        fun start(context: Context, replay: Boolean) {
            ...
            context.getSharedPreferences("sync-recovery", Context.MODE_PRIVATE).edit().putString("message", "操作已请求；重复点击会合并，等待网络条件或当前操作完成").apply()
        }
    }
}
