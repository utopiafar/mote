...
(async () => {
  ...
  await until(()=>js(`document.querySelectorAll('.timeline-group .capture-card').length===30`),'cursor pagination');
  ...
  await until(()=>js(`document.querySelector('.filter-count').innerText.includes('/ 26')`),'local day filter');
  ...
  await until(()=>js(`document.querySelector('.filter-count').innerText.includes('/ 1')&&document.querySelectorAll('.timeline-group .capture-card').length===1`),'OCR status filter');
  window.setSize(1360,1000);await sleep(100);
  await clickNav('资料库');
