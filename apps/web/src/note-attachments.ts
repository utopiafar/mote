...
export async function uploadNoteAttachment(api:Api,file:File,deviceId:string):Promise<string>{
  ...
  const sourceId='notes-'+deviceId.replace(/[^a-zA-Z0-9_.:-]/g,'').slice(0,100);
  await api.request('/api/sources',{method:'POST',body:JSON.stringify({id:sourceId,name:'Note attachments',kind:'upload',deviceId,platform:'import',retention:'archive',enabled:true})});
