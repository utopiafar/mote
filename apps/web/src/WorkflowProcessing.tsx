...
export function WorkflowProcessing({api,extensions=false,onNavigate}:{api:Api;extensions?:boolean;onNavigate:(page:Page)=>void}) {
  const [view,setView]=useState<View>(),[error,setError]=useState(''),[revision,setRevision]=useState(0),[busy,setBusy]=useState(''),[filter,setFilter]=useState('all'),[cursor,setCursor]=useState<number>();
  useEffect(()=>{const c=new AbortController();setError('');void api.request<View>('/api/processing?'+new URLSearchParams({...filter==='all'?{}:{state:filter},...cursor?{cursor:String(cursor)}:{}}),{signal:c.signal}).then(value=>{if(!c.signal.aborted)setView(value);}).catch(e=>{if(!c.signal.aborted)setError(errorMessage(e));});return()=>c.abort();},[api,revision,filter,cursor]);
  ...
  return <><div className="page-heading"><div className="eyebrow">MOTE / SYSTEM</div><h1>{moteText(extensions?'扩展能力':'处理任务')}</h1><p>{moteText(extensions?'已注册的处理器及其版本。扩展由节点管理，不提供在线安装。':'查看处理步骤、依赖与等待原因。重试保留其他已完成步骤。')}</p></div>
    ...
    {view&&!extensions&&<><nav className="section-tabs" aria-label={moteText('任务状态')}>{['all','waiting','running','blocked','failed','succeeded','cancelled','stale'].map(state=><button key={state} aria-current={filter===state?'page':undefined} onClick={()=>{setCursor(undefined);setFilter(state);}}>{state==='all'?moteText('全部'):moteText(states[state])}</button>)}</nav>
      ...
      <div className="processing-actions">{cursor&&<button className="button" onClick={()=>setCursor(undefined)}>{moteText('返回')}</button>}{view.nextCursor&&<button className="button" onClick={()=>setCursor(view.nextCursor!)}>{moteText('更多')}</button>}</div>
