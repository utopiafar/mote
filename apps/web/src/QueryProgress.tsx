...
export interface QueryRun {id:string;status:'running'|'completed'|'failed';createdAt:string;updatedAt:string;conversationId?:string;turnId?:string;events:{stage:'starting'|'model'|'tool'|'validating';at:string;tool?:string;message?:string;count?:number;phase?:'started'|'completed';step?:number}[];error?:{message:string}}
const tools:Record<string,string>={search_context:'检索上下文',timeline:'浏览时间线',evidence:'展开原始证据',activity:'核对采样时长',media_activity:'核对媒体活动',devices:'查看设备',sources:'查看资料来源',source_items:'读取来源记录',source_history:'查阅历史版本',memories:'检索记忆',file_chunks:'读取文件片段'};
export function progressLabel(e:QueryRun['events'][number]):string {
  if(e.message)return e.message;
  if(e.stage==='tool')return `${tools[e.tool??'']??'读取资料'}${e.phase==='started'?' · 正在执行':` · 返回 ${e.count??0} 项`}`;
  return {starting:'准备只读查询',model:`模型正在分析${e.step!==undefined?` · 第 ${e.step+1} 步`:''}，决定下一步检索或组织回答`,validating:'校验回答格式和证据引用'}[e.stage];
}
export function QueryProgress({run,error}:{run:QueryRun;error:string}) {
  ...
  return <section className="panel query-progress" aria-label="对话执行进度"><div className="section-heading"><h3>{run.status==='running'?<LoaderCircle size={18} className="spin"/>:run.status==='completed'?<CheckCircle2 size={18}/>:<AlertCircle size={18}/>} {run.status==='running'?'正在处理':run.status==='completed'?'回答已归档':'此次未完成'}</h3><span className="badge muted">{Math.floor(seconds/60)} 分 {seconds%60} 秒</span></div>
    ...
    <p className="fine-print">展示实际执行阶段与工具进度，不展示模型内部思维链。</p>
  </section>;
}
