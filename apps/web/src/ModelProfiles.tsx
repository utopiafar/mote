...
import {DEFAULT_AGENT_TIMEOUT_MS,DEFAULT_MODEL_MAX_TOKENS,DEFAULT_MODEL_REQUEST_TIMEOUT_MS,DEPLOYMENT_MODEL_PROFILE_ID,MODEL_FEATURE_LABELS,type ModelFeature,type ModelSettingsView,type ModelTestResult} from '@mote/shared/models';
...
export function ModelProfiles({api,revision,onApplied}:{api:Api;revision:number;onApplied:()=>void}){
  ...
  return <div className="provider-workspace">
    <aside className="panel provider-sidebar" aria-label={moteText("Provider 预设")}>
      ...
      <button className="button primary" disabled={busy||!view||atCapacity} onClick={()=>{if(!view)return;const id=crypto.randomUUID();void mutate('/api/model-settings/profiles/'+id,'PUT',{revision:view.revision,name:moteText("新 Provider 预设"),settings:{provider:'custom',protocol:'openai-completions',baseUrl:'',model:'',reasoningEffort:'auto',maxTokens:DEFAULT_MODEL_MAX_TOKENS,modelRequestTimeoutMs:DEFAULT_MODEL_REQUEST_TIMEOUT_MS,agentTimeoutMs:DEFAULT_AGENT_TIMEOUT_MS,allowUnauthenticatedLocal:false,apiKey:null,headers:null,extraBody:null}},id);}}><Plus size={16}/>{moteText("新增预设")}</button>
    ...
    <div className="provider-detail">
      ...
      {profile&&view&&<>
        ...
        {profile.readOnly?<section className="panel deployment-provider"><div className="section-heading"><h2><LockKeyhole size={18}/>{moteText("部署配置")}</h2><span className="badge muted">{moteText("只读")}</span></div><p>{moteText("来自本次启动读取的配置文件或环境变量。修改文件后重启生效；需要在页面调整时，请先复制为独立预设。")}</p><dl className="provider-facts"><div><dt>Provider</dt><dd>{profile.settings.provider}</dd></div><div><dt>{moteText("接口协议")}</dt><dd>{profile.settings.protocol}</dd></div><div><dt>{moteText("服务地址")}</dt><dd>{profile.settings.baseUrl||moteText("本机 Codex 登录")}</dd></div><div><dt>{moteText("默认模型")}</dt><dd>{profile.settings.model||moteText("未配置")}</dd></div><div><dt>API key</dt><dd>{profile.settings.apiKeyConfigured?moteText("已配置，不回显"):moteText("未配置")}</dd></div><div><dt>{moteText("单次模型请求 / Agent 总运行超时")}</dt><dd>{profile.settings.modelRequestTimeoutMs===null?moteText("不适用"):profile.settings.modelRequestTimeoutMs/1000+'s'} / {profile.settings.agentTimeoutMs===null?moteText("未设置"):profile.settings.agentTimeoutMs/1000+'s'}</dd></div></dl><button className="button subtle" disabled={busy||!profile.settings.model} onClick={()=>void testDeployment()}>{busy?moteText("正在测试…"):moteText("测试连接")}</button><small className="provider-test-note">{moteText("仅发送固定合成记录，验证模型与只读工具调用。")}</small>{probe&&<p role="status" className={'notice '+(probe.ok?'success':'error')}>{probe.message}</p>}</section>:<ModelSettingsEditor key={selected} api={api} revision={revision} onApplied={onApplied} profileId={selected}/>}
      </>}
    </div>
  </div>;
}
