...
export function SoftwareUpdate({api}: {api: Api}) {
  ...
  return <section className="panel software-update" aria-labelledby="software-update-title">
    <div className="section-heading"><div><h2 id="software-update-title">软件版本与更新</h2><p>更新替换程序，保留原环境设置、模型、队列和资料库。</p></div><button className="button subtle" disabled={busy} onClick={()=>void check()}><RefreshCw size={15} className={busy?'spin':''}/>{busy?'正在检查…':'检查新版本'}</button></div>
