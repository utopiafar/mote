...
const kinds:Record<string,string>={file:'文件处理',capture:'截图处理',memory:'记忆整理',workflow:'上下文处理',import:'资料导入',query:'问答',insight:'洞察',embedding:'检索向量计算'};
const steps:Record<string,string>={'files.pipeline':'内容提取','files.summary':'文件摘要','file-step':'格式处理','perception.ocr':'文字识别','perception.semantic':'图片理解','memory.batch':'记忆批次','imports.prepare':'分析原件','imports.commit':'保存记录','query.run':'问答','insight.run':'洞察','lifecycle-window':'后台整理窗口','actions.extract':'日程分析','embedding.capture':'记录向量索引','embedding.file':'文件片段向量索引','embedding.query':'检索向量计算'};
const destinations:Record<string,Page>={file:'files',capture:'timeline',memory:'memories',workflow:'timeline',import:'imports',query:'ask',insight:'insights',embedding:'ask'};
...
function Detail({api,id,onClose,onNavigate}:{api:Api;id:string;onClose:()=>void;onNavigate:(page:Page)=>void}){
 ...
 return <section className="panel" aria-label={moteText('任务详情')}><div className="processing-actions"><h2 ref={heading} tabIndex={-1}>{moteText('任务详情')}</h2><button className="button" onClick={onClose}>{moteText('关闭')}</button></div>
  ...
  <FeaturePanels api={api} onOpen={()=>{}} value={{kind:'mote.operation',schemaVersion:1,representation:'saved-record',ref:id,revision:String(data.operation.updatedAt),title:id,text:JSON.stringify(data,null,2)}}/><div className="processing-actions">{cursor&&<button className="button" onClick={()=>setCursor(undefined)}>{moteText('返回')}</button>}{data.nextCursor&&<button className="button" onClick={()=>setCursor(data.nextCursor!)}>{moteText('更多步骤')}</button>}<button className="button" onClick={()=>onNavigate(data.operation.id.startsWith('workflow:actions:')?'actions':destinations[data.operation.kind]??'timeline')}>{moteText('打开来源与处理操作')}</button></div></>}
 </section>;
}
