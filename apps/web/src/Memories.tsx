...
export function Memories({api,range,onOpen,refreshVersion=0,embedded=false}:{embedded?:boolean;refreshVersion?:number;api:Api;range:Range;onOpen:(id:string)=>void}){
  ...
  const [search,setSearch]=useState(''),[query,setQuery]=useState(''),[tier,setTier]=useState(''),[layer,setLayer]=useState('memory'),[cursor,setCursor]=useState<string>(),[nextCursor,setNextCursor]=useState<string|null>(null);
  ...
  useEffect(()=>{const timer=setInterval(()=>refresh(),10000);return()=>clearInterval(timer);},[]);
