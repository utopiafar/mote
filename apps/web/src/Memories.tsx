...
const codingLabels:Record<string,string>={pitfall:'踩坑经验',decision:'设计决策',principle:'可复用原则',preference:'编码偏好',session:'本次会话',project:'项目内适用',shared:'可跨项目参考',observed:'已观察',user_confirmed:'用户已确认',tested:'有测试证据',unverified:'尚未验证'};
...
export function Memories({api,range,onOpen,refreshVersion=0,embedded=false}:{embedded?:boolean;refreshVersion?:number;api:Api;range:Range;onOpen:(id:string)=>void}){
  ...
  async function download(){if(!detail)return;try{const blob=new Blob([`# ${detail.title}\n\n${detail.statement??''}\n\n## 判断边界\n\n${detail.uncertainty??''}\n\n## 原始证据\n\n${JSON.stringify(detail.evidence??[],null,2)}\n`],{type:'text/markdown'});const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='memory.md';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}catch(e){setError(errorMessage(e));}}
  ...
  return <section className="memories-page"><div className="page-heading split-heading"><div><div className="eyebrow">有依据，才值得记住</div><Heading>记忆</Heading><p>从资料中提取可复用的事实与经验。每条记忆都能回到原始证据，由你确认。</p></div><button className="button primary" disabled={busy||job?.status==='running'||job?.status==='queued'} onClick={()=>void extract()}>{busy?<LoaderCircle size={16} className="spin"/>:<Sparkles size={16}/>}提取当前范围的记忆</button></div>
    ...
    <form className="memory-search" onSubmit={e=>{e.preventDefault();setQuery(search);setCursor(undefined);}}><label>检索记忆<input aria-label="检索记忆" value={search} onChange={e=>setSearch(e.target.value)} placeholder="搜索中文或英文原文" maxLength={500}/></label><label>记忆层级<select value={tier} onChange={e=>{setTier(e.target.value);setCursor(undefined);}}><option value="">全部层级</option><option value="episode">事件记忆</option><option value="consolidated">长期记忆</option></select></label><button className="button subtle" type="submit">搜索</button></form>
    <nav className="segmented-nav memory-filters" aria-label="记忆状态">{[['all','全部'],['proposed','待确认'],['published','已确认'],['stale','需要重验']].map(([id,label])=><button key={id} className={filter===id?'active':''} onClick={()=>{setFilter(id);setCursor(undefined);}}>{label}</button>)}</nav>
    <div className="workspace-layout"><aside className="workspace-list" aria-label="记忆列表"><div className="workspace-list-heading"><h2>{filter==='all'?'全部记忆':labels[filter]}</h2><button className="icon-button" aria-label="刷新记忆" onClick={refresh}><RefreshCw size={15}/></button></div>{loading&&<p className="muted">正在读取…</p>}{!loading&&!visible.length&&<p className="muted">这里还没有记忆。导入资料后，可以开始提取。</p>}{visible.map(item=><button className={'workspace-select '+(detail?.id===item.id?'active':'')} key={item.id} onClick={()=>void open(item.id)}><strong>{item.title}</strong>{item.coding&&<small>编码经验 · {codingLabels[item.coding.kind]} · {codingLabels[item.coding.scope]}</small>}<span>{item.tier==='consolidated'?'长期记忆':'事件记忆'}</span><span className={'status-label '+(item.status==='stale'?'attention':'')}>{labels[item.status]||item.status}</span><small>{dateTime(item.createdAt)} · {item.evidenceCount??item.evidenceIds?.length??0} 条证据</small></button>)}<div className="memory-actions">{cursor&&<button className="button subtle" onClick={()=>setCursor(undefined)}>返回第一页</button>}{nextCursor&&<button className="button subtle" onClick={()=>setCursor(nextCursor)}>下一页</button>}</div></aside>
