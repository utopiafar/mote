  ...
  async function run(kind: 'save' | 'test' | 'restore') {
    ...
    try {
      ...
      if (kind === 'test') setProbe(result as ModelTestResult);
        ...
        api.setAgentTimeout(Math.max(snapshot.settings.timeoutMs,...(snapshot.profiles??[]).map(p=>p.settings.timeoutMs)));
        onApplied();
      }
    } catch (e) {
      if (controller.signal.aborted || requestEpoch !== epoch.current) return;
  ...
  return <>
    <section className="panel config-builder model-settings-editor" aria-label={moteText("模型服务设置")} aria-busy={busy}>
      ...
      {active && draft && <form onSubmit={e => {e.preventDefault(); void run('save');}}>
        <fieldset className="model-settings-fields" disabled={busy}>
          ...
          {draft.protocol==='codex-app-server'?<p className="preference-note">{moteText("使用中央服务器上 Codex CLI 的文件登录凭据。无需在这里填写 API key。Codex 仍会将选中的上下文发送到其模型服务；最长等待时间生效，输出 token 上限由 Codex 管理。")}</p>:<div className="model-credential-section"><div className="model-subheading"><h3><KeyRound size={16}/>API key</h3><span className={`badge ${active.snapshot.settings.apiKeyConfigured ? 'green' : 'muted'}`}>{active.snapshot.settings.apiKeyConfigured ? moteText("已配置，不回显") : moteText("未配置")}</span></div>
          ...
          <div className="preference-grid">
            ...
            <label className="preference-field">{moteText("最长等待时间（秒）")}<input aria-label={moteText("最长等待时间（秒）")} type="number" min={5} max={600} step={1} value={draft.timeoutSeconds} onChange={e => change({timeoutSeconds: e.target.value})}/><small>{moteText("5–600 秒，适用于模型请求。")}</small></label>
          </div>
    ...
    {active && saved && <section className="panel effective-settings model-effective-settings" aria-label={moteText("当前生效模型配置")}>
      ...
      {([
        ...
        [moteText("输出上限 / 等待时间"), moteText("{0} / {1} 秒", saved.protocol==='codex-app-server'?moteText("输出由 Codex 管理"):saved.maxTokens+' tokens', saved.timeoutMs / 1000)],
