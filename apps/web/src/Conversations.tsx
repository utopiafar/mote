...
export function Conversations({api, configured, devices, range, renderAnswer}: {
  ...
  return <div className="conversation-layout">
    <aside className="conversation-history panel" aria-label={moteText("对话历史")}>
      <div className="conversation-history-heading"><h2>{moteText("对话历史")}</h2><button className="icon-button" aria-label={moteText("刷新对话历史")} disabled={loading} onClick={() => void loadHistory()}><RefreshCw size={16} className={loading ? 'spin' : ''}/></button></div>
      <button className="button subtle full" disabled={busy} onClick={startNew}><Plus size={16}/>{moteText("新对话")}</button>
      ...
      <div className="conversation-list">{items.map(item => <button key={item.id} className={`conversation-item ${conversation?.id === item.id ? 'active' : ''}`} aria-current={conversation?.id === item.id ? 'true' : undefined} disabled={busy} onClick={() => void open(item.id)}>
        <strong>{item.title}</strong><span>{dateTime(item.updatedAt)} · {item.turnCount}{' '}{moteText("轮")}</span>
      </button>)}</div>
    ...
    <section className="conversation-content" aria-label={moteText("当前对话")} aria-busy={busy || opening}>
      <div className="conversation-heading"><div><h2>{conversation?.title ?? moteText("开始一段新对话")}</h2><p>{moteText("对话保存在中央节点。时间与设备筛选用于下一次提问。")}</p></div>{conversation && <button className="icon-button" aria-label={moteText("删除此对话")} disabled={busy} onClick={() => setConfirmDelete(value => !value)}><Trash2 size={17}/></button>}</div>
      ...
      {conversation?.turns.map(turn => <article className="answer-panel conversation-turn" key={turn.id}>
        ...
        {turn.result.modelSelection&&<small className="model-used">{turn.result.modelSelection.profileName} · {turn.result.modelSelection.model}</small>}
        {turn.evidenceDeleted ? <p className="notice">{moteText("相关证据已删除，这条历史回答已清除。可以继续提问查阅现有记录。")}</p> : renderAnswer(turn.result)}
      </article>)}
      ...
      <form className="ask-form" onSubmit={event => void submit(event)}>
        <textarea aria-label={moteText("向 Mote 提问")} placeholder={conversation ? moteText("接着问，Mote 会结合前面的对话。") : moteText("比如，我最近都在忙什么？")} value={question} onChange={event => setQuestion(event.target.value)} onKeyDown={event => {if (event.key === 'Enter' && (event.metaKey || event.ctrlKey)) void submit(event);}} disabled={busy || opening} maxLength={8000} rows={3}/>
        <div><span><ShieldCheck size={14}/>{moteText("只读查询 · 回答附带原始证据")}</span><button className="send-button" type="submit" disabled={busy || opening || !question.trim() || !configured} aria-label={moteText("发送问题")}>{busy ? <LoaderCircle className="spin" size={19}/> : <ArrowUp size={19}/>}</button></div>
      </form>
