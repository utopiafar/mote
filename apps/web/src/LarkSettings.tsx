...
export function LarkSettings({api,onBack,onSources}:{api:Api;onBack:()=>void;onSources:()=>void}){
 ...
 const read=useResource<LarkStatus>(api,'/api/connectors/lark',2000),scope=useRef(new AbortController());
 const request=async<T,>(path:string,init?:RequestInit):Promise<T>=>{const signal=scope.current.signal;const result=init?.method?await api.request<T>(path,{...init,signal}):await readResource<T>(api,path,signal);signal.throwIfAborted();return result;};
 const accept=(next:LarkStatus,reset=false)=>{if(wasConnected.current&&!next.connected){resources(api).invalidate(key=>key==='/api/connectors/lark/calendars');dirty.current=false;setCalendars([]);setCalendarLoaded(false);}wasConnected.current=next.connected;setStatus(next);if(reset||!dirty.current){setSelection(next.selection);setDocuments(next.selection.documents.join('\n'));if(reset)dirty.current=false;}};
 useEffect(()=>{
  const controller=new AbortController();scope.current=controller;alive.current=true;dirty.current=false;wasConnected.current=false;setStatus(undefined);setSelection(emptySelection);setDocuments('');setCalendars([]);setCalendarLoaded(false);setError('');setNotice('');setSecret('');setBusy(false);
  void request<LarkStatus>('/api/connectors/lark/check',{method:'POST'}).then(()=>read.refresh()).catch(e=>{if(!controller.signal.aborted)setError(requestError(e));});
  return()=>{alive.current=false;controller.abort();};
 },[api]);
 useEffect(()=>{if(read.data)accept(read.data);if(read.error instanceof ApiError&&[401,403,404,410].includes(read.error.status)){setStatus(undefined);setCalendars([]);setCalendarLoaded(false);setDocuments('');setSelection(emptySelection);dirty.current=false;}},[read.data,read.error]);
 const perform=async(fn:()=>Promise<void>)=>{const signal=scope.current.signal;setBusy(true);setError('');setNotice('');try{await fn();if(!signal.aborted)read.refresh();}catch(e){if(alive.current&&!signal.aborted)setError(requestError(e));}finally{if(alive.current&&!signal.aborted)setBusy(false);}};
 const command=(name:string)=>void perform(async()=>{await request(`/api/connectors/lark/${name}`,{method:'POST'});accept(await request<LarkStatus>('/api/connectors/lark'));});
 ...
 return <div className="lark-settings">
  ...
  {status&&<>
   ...
   <section className="panel lark-step"><h2>3 · {moteText('选择同步范围')}</h2><fieldset disabled={running||!status.connected}><label>{moteText('文档链接（每行一个，最多 30 个）')}<textarea rows={4} value={documents} onChange={e=>{dirty.current=true;setDocuments(e.target.value);}} placeholder="https://example.feishu.cn/docx/…"/></label><p className="fine-print">{moteText('支持 docx 和 wiki 文档正文；嵌入表格、附件和图片仅保留正文中的引用，不自动下载。')}</p>
    <div className="section-heading"><h3>{moteText('日历')}</h3><button className="button subtle" type="button" onClick={()=>void perform(async()=>{resources(api).invalidate(key=>key==='/api/connectors/lark/calendars');const data=await request<{calendars:LarkCalendar[]}>('/api/connectors/lark/calendars');setCalendars(data.calendars);setCalendarLoaded(true);})}>{moteText('加载可选日历')}</button></div>
    ...
    <button className="button" onClick={()=>void perform(async()=>{const next=await request<LarkStatus>('/api/connectors/lark/selection',{method:'PUT',body:JSON.stringify({...selection,documents:documents.split('\n').map(v=>v.trim()).filter(Boolean)})});accept(next,true);setNotice(moteText('同步范围已保存。'));})}>{moteText('保存同步范围')}</button>
   </fieldset></section>
   <section className="panel lark-step"><h2>4 · {moteText('同步与归档')}</h2><p>{status.lastSyncAt?`${moteText('最近成功同步：')}${new Date(status.lastSyncAt).toLocaleString()}`:moteText('还没有完成过同步。')}</p><div className="lark-buttons"><button className="button" disabled={running||!status.connected||dirty.current||!(status.selection.documents.length||status.selection.calendarIds.length)} onClick={()=>command('sync')}>{moteText('立即同步')}</button><button className="button subtle" onClick={onSources}>{moteText('查看来源与归档')}</button><button className="button subtle" disabled={busy} onClick={()=>setDisconnecting(true)}>{moteText('断开连接')}</button></div>{dirty.current&&<p className="fine-print">{moteText('请先保存修改后的同步范围。')}</p>}
    {disconnecting&&<div className="notice" role="alert"><p>{moteText('断开会停止读取并暂停这些来源，保留已归档资料。飞书端授权及 CLI 凭据仍保留，可在飞书中单独撤销。')}</p><button className="button subtle" disabled={busy} onClick={()=>setDisconnecting(false)}>{moteText('返回')}</button><button className="button" disabled={busy} onClick={()=>void perform(async()=>{accept(await request<LarkStatus>('/api/connectors/lark',{method:'DELETE'}),true);setDisconnecting(false);setNotice(moteText('连接已断开，历史归档已保留。'));})}>{moteText('确认断开')}</button></div>}
   </section>
  </>}
 </div>;
}
