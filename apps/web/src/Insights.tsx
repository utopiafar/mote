...
export function Insights({api,range,configured,onOpen,onSettings,onChanged,refreshVersion=0}:{refreshVersion?:number;api:Api;range:Range;configured:boolean;onOpen:(id:string)=>void;onSettings:()=>void;onChanged:()=>void}){
  ...
  useEffect(()=>{
    ...
    const poll=async()=>{
      try{
        ...
        else if(next.status==='completed'){setError('这份报告已因原始资料变化而失效，请重新生成回顾。');}
  ...
  async function generate(){
    ...
    const body=JSON.stringify({...(!allTime?range:{}),timeZone:Intl.DateTimeFormat().resolvedOptions().timeZone,prompt:prompt.trim()||undefined});
  ...
  return <section className="insights-page"><div className="page-heading split-heading"><div><div className="eyebrow">从记录里发现值得回看的事</div><h1>洞察</h1><p>让 Mote 阅读你的资料，生成带有证据的回顾与分析。</p></div><button className="button primary" disabled={busy} onClick={()=>{setCreating(true);setSelected('');setError('');setRun(null);pendingRequest.current=null;}}><Plus size={16}/>新建洞察</button></div>
    {error&&<div className="error-banner" role="alert">{error}</div>}
    <div className="workspace-layout"><aside className="workspace-list" aria-label="洞察历史"><div className="workspace-list-heading"><h2>过去的洞察</h2><span>{items.length}</span></div>{loading&&<p className="muted">正在读取…</p>}{!loading&&!items.length&&<p className="muted">第一份洞察会保存在这里，之后可以随时回看。</p>}{runs.filter(item=>item.status!=='completed').map(item=><button className="workspace-select" key={item.id} disabled={busy&&item.id!==run?.id} onClick={()=>{setRun(item);setCreating(true);setSelected('');setReconnect(n=>n+1);}}><strong>{item.status==='running'?'正在回顾…':'回顾未完成'}</strong><small>{dateTime(item.createdAt)}</small>{item.error&&<span>{item.error.message}</span>}</button>)}{items.map(item=><button className={'workspace-select '+(!creating&&selected===item.runId?'active':'')} key={item.id||item.runId} onClick={()=>{setCreating(false);setSelected(item.runId);}}><strong>{item.artifact?.title||'个人回顾'}</strong><span className="insight-excerpt">{answerPreview(item,80)}</span><small>{item.createdAt?dateTime(item.createdAt):'刚刚'} · {item.citations.length} 条证据</small></button>)}</aside>
      <div className="workspace-content">{run&&<InsightProgress run={run} pollError={pollError} elapsed={Math.max(0,Math.floor(((run.status==='running'?now:Date.parse(run.updatedAt))-Date.parse(run.createdAt))/1000))} onRetry={()=>{setRun(null);pendingRequest.current=null;setCreating(true);setError('');}} onResume={()=>setReconnect(n=>n+1)}/>} {creating?<form className="panel insight-compose" onSubmit={e=>{e.preventDefault();void generate();}}><div className="compose-mark"><Sparkles size={26}/></div><h2>你想从这些记录中看见什么？</h2><p className="muted">可以给一个方向，也可以直接生成这段时间的个人回顾。</p><label className="field-label">关注的方向 <span className="muted">选填</span><textarea disabled={busy} rows={5} maxLength={8000} value={prompt} onChange={e=>setPrompt(e.target.value)} placeholder="例如：回看我最近的项目推进，找出反复出现的阻碍，并列出值得跟进的线索。"/></label><label className="field-label">资料范围<select disabled={busy} value={allTime?'all':'selected'} onChange={e=>setAllTime(e.target.value==='all')}><option value="selected">页面上方选择的时间范围</option><option value="all">全部已归档资料</option></select><small>{allTime?'使用中央归档中的所有可检索资料。':range.after?`从 ${dateTime(range.after)}${range.before?' 至 '+dateTime(range.before):' 至现在'}`:'当前没有时间限制。'}</small></label>
      ...
      <div className="form-footer"><span className="muted">结论附带引用，原始资料随时可查。</span><button className="button primary" disabled={busy||!configured}>{busy?<LoaderCircle size={16} className="spin"/>:<ArrowRight size={16}/>} {launching?'正在启动…':busy?'回顾正在运行…':'生成个人回顾'}</button></div>{launching&&<p role="status" className="muted">正在向中央节点提交回顾任务…</p>}
