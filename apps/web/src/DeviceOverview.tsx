import {ArrowLeft, ArrowRight, CheckCircle2, Clock3, HardDrive, Info, Monitor, Plus, Smartphone} from 'lucide-react';
...
export function DeviceOverview({devices,onConnect}:{devices:Device[];onConnect:()=>void}) {
 return <div className="device-overview"><div className="page-heading split-heading"><div><div className="eyebrow">记录各自发生，资料在此汇合</div><h1>你的设备</h1><p>采集在设备本机运行，同步按每台设备的偏好进行。</p></div><button className="button primary" onClick={onConnect}><Plus size={17}/>添加设备</button></div>
 <div className="device-observation-note"><Clock3 size={18}/><p>这里显示设备最近一次上报。较久未联系可能是定时、手动同步或网络变化，不能据此判断本机是否正在采集。</p></div>
 {!devices.length?<section className="panel device-empty"><Monitor size={34}/><h2>从你的第一台设备开始</h2><p>生成一次性邀请，在手机或 Mac 的连接设置中确认。客户端也可以先在本机记录，稍后再连接。</p><button className="button primary" onClick={onConnect}>生成连接邀请<ArrowRight size={16}/></button></section>:<div className="device-grid">{devices.map(device=>{const fresh=deviceState(device)!=='stale',Icon=device.platform==='android'?Smartphone:device.platform==='import'?HardDrive:Monitor;return <article className="panel device-card" key={device.deviceId}>
