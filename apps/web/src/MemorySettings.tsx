...
export function MemorySettings({api}:{api:Api}){
  ...
  return <section className="panel memory-settings"><div className="section-heading"><div><h2>{moteText("记忆与洞察的节奏")}</h2><p>{moteText("周期已到，并且新增变化达到门槛时才运行。每分钟检查一次；保存后直接生效，进行中的一轮沿用启动时的设置。")}</p></div></div>
