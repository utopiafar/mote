...
export function PerceptionSettings({api}:{api:Api}){
 ...
 <label>{moteText("语义理解时机")}<select value={settings.semanticMode} onChange={e=>change('semanticMode',e.target.value as Settings['semanticMode'])}><option value="manual">{moteText("手动处理（默认）")}</option><option value="realtime">{moteText("实时后台处理")}</option><option value="batch">{moteText("分批处理")}</option></select></label>
 <label>{moteText("分批等待（分钟）")}<input type="number" min="1" max="1440" value={settings.batchMinutes} onChange={e=>change('batchMinutes',e.target.valueAsNumber)}/></label>
 <label>{moteText("每轮最多处理数")}<input type="number" min="1" max="100" value={settings.batchSize} onChange={e=>change('batchSize',e.target.valueAsNumber)}/></label>
