...
import {environmentFragment,effectiveValues,matchingProvider} from '../src/configuration-draft.js';
...
test('effective draft defaults preserve custom values, convert MiB, and never reproduce secret status as credentials',()=>{
 ...
 assert.equal(matchingProvider('https://private.example/v1'),'custom');
 assert.equal(matchingProvider('https://api.deepseek.com/'),'deepseek');
});
